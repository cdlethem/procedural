/** Example-private mirror of the web Loop Marks construction and drawing. */
import { closedSpline2D } from "../../src/closed-spline.js";
const WORK_LIMIT = 12e3;
...
function member(params, key, values) {
  const value = params[key];
  if (!values.includes(value)) throw new Error(`${key} must be one of ${values.join(", ")}`);
  return value;
}
function options(params) {
  const layout = member(params, "layout", ["row", "grid", "nested"]);
  const treatment = member(params, "treatment", ["outline", "tiles", "fans", "outline-tiles"]);
  const tileShape = member(params, "tileShape", ["bar", "diamond", "tick"]);
  return {
    layout,
    treatment,
    tileShape,
    ...
    nestedScale: number(params, "nestedScale", 0.2, 1),
    knotCount: number(params, "knotCount", 5, 16, true),
    lobes: number(params, "lobes", 0, 12, true),
    lobeDepth: number(params, "lobeDepth", 0, 0.8),
    ...
    outlineWeight: number(params, "outlineWeight", 0.1, 12),
    fanOpacity: number(params, "fanOpacity", 0, 255)
  };
}
function validateLoopMarks(params) {
  if (params.legacy === true) return;
  buildLoopMarks(params);
}
function centerAt(q, index) {
  if (q.layout === "row")
    return [[
      q.centerX + (index - (q.loopCount - 1) / 2) * q.spacingX,
      q.centerY + (index - (q.loopCount - 1) / 2) * q.spacingY
    ], 1];
  ...
  return [[
    q.centerX + (column - (rowCount - 1) / 2) * q.spacingX,
    q.centerY + (row - (rows - 1) / 2) * q.spacingY
  ], 1];
}
function buildLoopMarks(params) {
  const q = options(params), loops = [], tileCounts = [];
  ...
  for (let index = 0; index < q.loopCount; index++) {
    ...
    const controls = Array.from({ length: q.knotCount }, (_, knot) => {
    ...
    const points = Array.from({ length: sampleCount }, (_, sample) => {
      ...
      const modulation = q.lobes === 0 ? 1 : 1 + q.lobeDepth * Math.cos(q.lobes * angle + q.phase * Math.PI / 180 + index * 2 * Math.PI / q.loopCount);
...
function colour(p, layer, index, alpha, kind) {
...
function polygon(p, vertices) {
  p.beginShape();
  for (const vertex of vertices) p.vertex(vertex[0], vertex[1]);
  p.endShape(p.CLOSE);
}
function drawLoopMarksModern(p, layer) {
  ...
  for (let index = 0; index < loops.length; index++) {
    ...
    if (drawsOutline) {
      ...
      colour(p, layer, index, 220, "stroke");
      for (let sample = 0; sample < points.length; sample++) {
        const a = points[sample], b = points[(sample + 1) % points.length];
    ...
    if (drawsTiles) {
      for (let tile = 0; tile < tileCounts[index]; tile++) {
        ...
        if (q.tileShape === "tick") {
          ...
          const v = (u, v2) => [x + tx * u + nx * v2, y + ty * u + ny * v2];
          polygon(p, q.tileShape === "bar" ? [v(-w, -h), v(w, -h), v(w, h), v(-w, h)] : [v(0, -h), v(w, 0), v(0, h), v(-w, 0)]);
        }
      }
    }
  }
