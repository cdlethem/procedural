...
const slug = "embossed-field";
const defaults = materialsASettings[slug].defaults;
const art = document.querySelector("#art");
const controls = document.querySelector("#controls");
const status = document.querySelector("#status");
const fields = Object.fromEntries(["source", "axis", "treatment", "scale", "gain", "seed", "palette"]
...
function readFields() {
  ...
  const scale = Number(fields.scale.value), gain = Number(fields.gain.value), seed = Number(fields.seed.value);
  if (!Number.isInteger(scale) || scale < 4 || scale > 120 || !Number.isFinite(gain) || gain < 0 || gain > 20 || !Number.isInteger(seed) || seed < 0 || seed > 4294967295) {
    status.textContent = "Use pixel size 4–120, relief strength 0–20, and a nonnegative integer seed.";
    return false;
  }
  Object.assign(state, {
    source: fields.source.value, axis: fields.axis.value, treatment: fields.treatment.value,
    scale, gain, seed, palette: Number(fields.palette.value),
  });
  return true;
}
  ...
  p.draw = () => {
    try {
      ...
      drawEmbossedField(p, {
        ...
        params: { scale: state.scale, gain: state.gain, source: state.source, axis: state.axis, treatment: state.treatment },
      });
      art.dataset.renderStatus = "ready";
      art.dataset.revision = String(++revision);
      status.textContent = `${state.source} · ${state.axis} response · ${state.treatment} · pixel size ${state.scale} · strength ${state.gain}`;
    } catch (error) {
...
controls.addEventListener("change", () => { if (readFields()) render(); });
...
syncFields();
window.embossedFieldStudy = { snapshot: () => ({ ...state, revision, renderStatus: art.dataset.renderStatus }) };
