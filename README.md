...
PointerMarks adds interactive supplied targets with deterministic replay and fixed-wire transfer.
BodyMarks adds tapered bodies and centerlines from retained backward field traces.
...
Ports of these newer workflows to JavaScript, py5 and Android are deferred. The accepted Java 0.33.0 source bundle
contains 35 editable workflows and 30 reusable operations. It is not a published registry
...
The accepted local archive is identified in [the Java 0.33 review](evidence/distribution/path-clip-java-review.json).
...
[Build and install the Java library](docs/building-java-from-source.md) for all 35 workflows.
