...
The source-bundle builder assembles Java0.24 directly from the checked-in sources, including
24 operations and 24 editable starters. PullMarks adds localized geometry warping; DepthMarks adds 3D fields and mesh coloring. PanelMarks adds irregular retained panels to the
closed-curve, noise-band, field, path, mesh and other workflows. The Java0.24 artifact
review is recorded in `evidence/distribution/cp21-java-review.json`. Each generated report records assembly
...
project assets. The font is needed to preserve the complete24-starter bundle; it is not
