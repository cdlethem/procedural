...
The source-bundle builder assembles Java0.21 directly from the checked-in sources, including
21 operations and 21 editable starters. DepthMarks adds3D fields and mesh coloring. PanelMarks adds irregular retained panels to the
closed-curve, noise-band, field, path, mesh and other workflows. The Java0.21 artifact
review is recorded in `evidence/distribution/cp18-java-review.json`. Each generated report records assembly
...
project assets. The font is needed to preserve the complete21-starter bundle; it is not
