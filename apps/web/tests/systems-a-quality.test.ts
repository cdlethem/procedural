...
test("ranked stripe fill keeps source switching nonempty and deterministic", () => {
  ...
  const base = layer("reaction-stripes", { source: "speckle", occupancy: .1, passes: 0 });
  for (const candidate of [
    layer("reaction-stripes", { source: "speckle", occupancy: .1, sourceX: .21, passes: 0 }),
    layer("reaction-stripes", { source: "speckle", occupancy: .1, sourceY: -.17, passes: 0 }),
    layer("reaction-stripes", { source: "speckle", occupancy: .1, frequency: 7, passes: 0 }),
    { ...base, seed: 43 },
  ]) assert.notDeepEqual(cellularInitialCells("reaction-stripes", candidate.params, candidate.seed), fields[2]);
