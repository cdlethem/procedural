...
export const externalDynamicsDefinitions:StudioDefinition[]=[
  ...
  {id:'sensing-trails',title:'Sensing trails',description:'Paired probes turn agents through a replaceable scalar field.',parameters:[numeric('ticks','Ticks','Replay paired sensing and motion.',0,80,1),choice('field','Field source','Replay the agents through a different scalar field.',['upper-right','lower-left']),numeric('gain','Turn gain','Signed response to right-minus-left field samples.',-.1,.1,.01),numeric('reach','Reach','Probe distance ahead of each agent.',4,40,1),toggle('dotMarks','Dot marks','Stamp sampled trail points instead of joining them.')],defaults:{ticks:40,field:'upper-right',gain:.05,reach:13,dotMarks:false}},
...
function sensorModel(ticks:number,kind:string,gain:number,reach:number){const field=retained(`sensor-field:${kind}`,()=>sensorField(kind));let agents=Array.from({length:72},(_,i)=>({position:[80+i%12*40,100+Math.floor(i/12)*60],headingTurns:i/72,speed:1.35}));let paths=agents.map(a=>[a.position]);
  for(let tick=0;tick<ticks;tick++){agents=sensorMotorStep2D({agents,field,sensorDistance:reach,sensorAngleTurns:.105,turnGain:gain,dt:1,maxWork:72*3}).agents;paths=paths.map((path,i)=>[...path.slice(-110),agents[i].position]);}
  return {agents,paths,field};}
function drawSensors(p:any,l:Layer){const q=l.params,m=retained(`sensors:${q.ticks}:${q.field}:${q.gain}:${q.reach}`,()=>sensorModel(Number(q.ticks),String(q.field),Number(q.gain),Number(q.reach)));
  scale(p,640,()=>{p.noStroke();for(let y=0;y<64;y+=2)for(let x=0;x<64;x+=2){const a=m.field.values[y*64+x];fill(p,l,2,Math.min(70,a*45));p.rect(x*640/63,y*640/63,640/63+.5,640/63+.5);}
    color(p,l,3,230);p.strokeWeight(1.6);p.noFill();for(const path of m.paths){if(q.dotMarks){for(let i=0;i<path.length;i+=8)p.point(...path[i]);}else polyline(p,path);}
    p.noStroke();fill(p,l,4);for(const a of m.agents)p.circle(...a.position,3.6);
  });}
...
function hatchModel(spacing:number,cross:number,rotation:number,twist:number,region:string){const source=islandRegions[region==='island-b'?1:0];const regionData={outer:source.outer.map(point=>[...point]),holes:source.holes.map(ring=>ring.map(point=>[...point]))};const r=rotation*Math.PI/180,t=twist*Math.PI/180;const directions=[[Math.cos(r),Math.sin(r)],[Math.cos(r+t),Math.sin(r+t)]];const layers=directions.map((direction,i)=>hatchRegionLines2D({region:regionData,origin:[0,0],direction,spacing:i?cross:spacing,phase:i?13:7,maxWork:1000000,maxOutputPaths:1000}));return {region:regionData,layers};}
...
function drawHatches(p:any,l:Layer){const q=l.params,m=retained(`hatches:${q.spacing}:${q.cross}:${q.rotation}:${q.twist}:${q.region}`,()=>hatchModel(Number(q.spacing),Number(q.cross),Number(q.rotation),Number(q.twist),String(q.region)));
...
function initialBridge(strain:number){const nodes:any[]=[],edges:any[]=[];let edgeId=0;for(let strand=0;strand<7;strand++){let previous:number|undefined;for(const y of BRIDGE_LEVELS){const x=78+strand*80+strain*Math.sin(y*.015+strand*1.1)+strain*.52*Math.cos(y*.029+strand*.65);const id=nodes.length;nodes.push({id,point:[x,y]});if(previous!==undefined)edges.push({id:edgeId++,a:previous,b:id});previous=id;}}return {nodes,edges,nextNodeId:nodes.length,nextEdgeId:edgeId};}
const bridgeCandidate=(tick:number,slant:number)=>{const y=110+tick*9.8,value=slant*(1+(23/31)*Math.sin(tick*.53));return [[24,y-value],[616,y+value]];};
const bridgeGap=(tick:number,stride:number)=>(tick*stride)%6;
...
function bridgeSeed(strain:number):BridgeState{const graph=initialBridge(strain);return {graph,kinds:new Map<number,string>(graph.edges.map(edge=>[edge.id,'strand'])),lastEvents:[]};}
function bridgeStep(state:BridgeState,tick:number,slant:number,stride:number):BridgeState{
  const result=insertSegmentBridge2D({graph:state.graph,candidate:bridgeCandidate(tick,slant),gapIndex:bridgeGap(tick,stride),maxNodes:160,maxEdges:200,maxWork:90000});
...
function bridgeModel(ticks:number,strain:number,slant:number,stride:number){
  const state=replayed(`bridge:${strain}:${slant}:${stride}`,ticks,()=>bridgeSeed(strain),(current,tick)=>bridgeStep(current,tick,slant,stride));
  return {...state,candidate:bridgeCandidate(ticks,slant)};
}
function drawBridge(p:any,l:Layer){const q=l.params,m=bridgeModel(Number(q.ticks),Number(q.strain),Number(q.slant),Number(q.stride));
...
function drawNeighborhood(p:any,l:Layer){const q=l.params,m=retained(growthModelKey(q),
 ...
 function elasticStep(state:any,seedNodeCount:number,growth:number,curl:number,windX:number,range:number,strength:number){
   const restGrowth=state.curves.map((curve:any)=>curve.restLengths.map((length:number)=>length*growth));
   const turnRates=state.curves.map((curve:any)=>curve.restTurns.map((_:number,index:number)=>{const id=curve.nodeIds[index+1];if(id>=seedNodeCount)return 0;const direction=curve.id===1?-1:1,profile=.7+.3*Math.sin(Math.PI*(id%10)/9);return direction*curl*profile;}));
   const externalAccelerations=state.nodes.map(()=>[windX,0]);
   return elasticCurveGrowStep2D({state,restGrowth,turnRates,externalAccelerations,stretchStiffness:.03,bendStiffness:80,contactRange:range,contactStrength:strength,damping:.88,dt:.4,maxSpeed:6,maxSegmentLength:42,maxNodes:160,maxEdges:160,maxWork:400000,maxBacktracks:8}).state;
 }
 function elasticModel(ticks:number,growth:number,curl:number,windX:number,range:number,strength:number){
   const seed=retained('elastic-seed',initialElastic);
   const state=replayed(`elastic:${growth}:${curl}:${windX}:${range}:${strength}`,ticks,()=>seed,current=>elasticStep(current,seed.nodes.length,growth,curl,windX,range,strength),1,48);
   return {state,seed};
 }
 function drawElastic(p:any,l:Layer){const q=l.params,m=elasticModel(Number(q.ticks),Number(q.growth),Number(q.curl),Number(q.windX),Number(q.range),Number(q.strength));
  scale(p,640,()=>{p.strokeCap(p.ROUND);const initial=new Map<number,number[]>(m.seed.nodes.map((node:any)=>[node.id,node.position]));const current=new Map<number,number[]>(m.state.nodes.map((node:any)=>[node.id,node.position]));
    color(p,l,2,80);p.strokeWeight(1.2);for(const curve of m.seed.curves)for(let i=1;i<curve.nodeIds.length;i++)p.line(...initial.get(curve.nodeIds[i-1])!,...initial.get(curve.nodeIds[i])!);
     for(const curve of m.state.curves){color(p,l,0);p.strokeWeight(4.5);for(let i=1;i<curve.nodeIds.length;i++)p.line(...current.get(curve.nodeIds[i-1])!,...current.get(curve.nodeIds[i])!);
       color(p,l,curve.id+1);p.strokeWeight(2.2);for(let i=1;i<curve.nodeIds.length;i++)p.line(...current.get(curve.nodeIds[i-1])!,...current.get(curve.nodeIds[i])!);}
     if(q.structure){p.noStroke();fill(p,l,4);for(const node of m.state.nodes)p.circle(...node.position,node.id<m.seed.nodes.length?4:2.5);}
     p.noStroke();for(const curve of m.state.curves){fill(p,l,4);p.circle(...current.get(curve.nodeIds[0])!,8);fill(p,l,curve.id+1);p.circle(...current.get(curve.nodeIds.at(-1))!,7);}
  });}
