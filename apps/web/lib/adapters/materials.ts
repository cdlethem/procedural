...
import { materialsADefinitions } from "./materials-a";
import { drawDiffusionEngraving, drawDitheredRibbons, drawOrderedHalftone, drawBayerWeave, drawEmbossedField, drawSignedEdgePrint, drawDistanceHalos, drawNearestFeatureMosaic, drawErodedLace, drawDilatedStamps, drawPerceptualBands, drawOklabOrbits, drawReducedMosaic, drawQuantizedStripes } from "../../../../packages/javascript/examples/materials-a-studies.js";
...
export function drawMaterials(p: any, layer: Layer): void {
 ...
 case "nearest-feature-mosaic": return drawNearestFeatureMosaic(p, layer);
 case "eroded-lace": return drawErodedLace(p, layer);
 case "dilated-stamps": return drawDilatedStamps(p, layer);
 case "perceptual-bands": return drawPerceptualBands(p, layer);
 case "oklab-orbits": return drawOklabOrbits(p, layer);
 case "reduced-mosaic": return drawReducedMosaic(p, layer);
 case "quantized-stripes": return drawQuantizedStripes(p, layer);
