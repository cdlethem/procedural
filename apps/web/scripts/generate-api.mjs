...
const bindings = {
  ...
  "binary-morphology-2d": ["binary-morphology-2d", "binaryMorphology2D"],
  "chaikin-polyline-2d": ["chaikin-polyline-2d", "chaikinPolyline2D"],
  "convex-hull-2d": ["convex-hull-2d", "convexHull2D"],
  "convolve-2d-signed": ["convolve-2d-signed", "convolve2DSigned"],
  "cost-grid-paths-2d": ["cost-grid-paths-2d", "costGridPaths2D"],
  "damped-wave-step-2d": ["damped-wave-step-2d", "dampedWaveStep2D"],
  "elementary-cellular-rows": ["elementary-cellular-rows", "elementaryCellularRows"],
  "euclidean-distance-transform-2d": ["euclidean-distance-transform-2d", "euclideanDistanceTransform2D"],
  "extrude-simple-polygon-3d": ["extrude-simple-polygon-3d", "extrudeSimplePolygon3D"],
  "floyd-steinberg-dither": ["floyd-steinberg-dither", "floydSteinbergDither"],
  "gray-scott-step-2d": ["gray-scott-step-2d", "grayScottStep2D"],
  "life-like-step-2d": ["life-like-step-2d", "lifeLikeStep2D"],
  "lloyd-relaxation-2d": ["lloyd-relaxation-2d", "lloydRelaxation2D"],
  "loop-subdivide-triangles-3d": ["loop-subdivide-triangles-3d", "loopSubdivideTriangles3D"],
  "median-cut-quantize": ["median-cut-quantize", "medianCutQuantize"],
  "offset-polyline-2d": ["offset-polyline-2d", "offsetPolyline2D"],
  "oklab-ramp": ["oklab-ramp", "oklabRamp"],
  "parallel-token-rewrite": ["parallel-token-rewrite", "parallelTokenRewrite"],
  "parallel-transport-ribbon-3d": ["parallel-transport-ribbon-3d", "parallelTransportRibbon3D"],
  "poisson-disc-2d": ["poisson-disc-2d", "poissonDisc2D"],
  "rk4-vector-grid-trace-2d": ["rk4-vector-grid-trace-2d", "rk4VectorGridTrace2D"],
  "scalar-grid-curl-2d": ["scalar-grid-curl-2d", "scalarGridCurl2D"],
  "seeded-depth-first-spanning-tree": ["seeded-depth-first-spanning-tree", "seededDepthFirstSpanningTree"],
  "simplify-polyline-2d": ["simplify-polyline-2d", "simplifyPolyline2D"],
  "skyline-pack-2d": ["skyline-pack-2d", "skylinePack2D"],
  "token-turtle-2d": ["token-turtle-2d", "tokenTurtle2D"],
  "triangulate-simple-polygon-2d": ["triangulate-simple-polygon-2d", "triangulateSimplePolygon2D"],
  ...
  "binary-cell-partition-2d": [
    ...
    "binaryCellPartition2D",
  ],
  "clip-segments-simple-polygon-2d": [
    "segment-clip",
    "clipSegmentsSimplePolygon2D",
  ],
  "closed-spline-2d": ["closed-spline", "closedSpline2D"],
  "cyclic-palette": ["cyclic-palette", "cyclicPalette"],
  "delaunay-2d": ["delaunay", "delaunay2D"],
  "gradient-noise-2d-01": ["gradient-noise-2d-01", "gradientNoise2D01"],
  "gradient-noise-3d-01": ["gradient-noise-3d-01", "gradientNoise3D01"],
  "gradient-path": ["gradient-path", "gradientPath2D"],
  "masked-source-over": ["masked-source-over", "maskedSourceOver2D"],
  "nearest-segment-contact-2d": [
    "nearest-segment-contact",
    "nearestSegmentContact2D",
  ],
  "noise-band-path": ["noise-band-path", "noiseBandPath2D"],
  "occupied-lattice-paths-2d": [
    "occupied-lattice-paths",
    "occupiedLatticePaths2D",
  ],
  "ordered-circle-filter": ["circle-placements", "orderedCircleFilter2D"],
  "ordered-convex-polygon-filter-2d": [
    "convex-polygon-placements",
    "orderedConvexPolygonFilter2D",
  ],
  "radial-profile-surface": ["radial-profile", "RadialProfile3D"],
  "radial-pull-2d": ["radial-pull", "radialPull2D"],
  "raster-crossfade": ["raster-crossfade", "rasterCrossfade2D"],
  "regular-grid": ["regular-grid", "regularGrid"],
  "retained-rectangle-cuts-2d": [
    "retained-rectangle-cuts",
    "retainedRectangleCuts2D",
  ],
  "seeded-circle-placement": ["circle-placements", "seededCirclePlacement2D"],
  "seeded-endpoint-branches": ["branch-tree", "seededEndpointBranches2D"],
  "seeded-line-pool-2d": ["line-pool", "seededLinePool2D"],
  "seeded-quadrant-partition": [
    "quadrant-partition",
    "seededQuadrantPartition2D",
  ],
  "seeded-triangle-points": ["triangle-points", "seededTrianglePoints2D"],
  "separable-blur-2d": ["separable-blur", "separableBlur2D"],
  "sequential-disc-projection-2d": [
    "disc-projection",
...
const entries = readdirSync(join(catalogRoot, "catalog/operations"))
  ...
  .map((name) => {
    ...
    if (binding && typeof packageBindings[binding[1]] !== "function")
      throw Error(
        `JavaScript binding drift: ${catalog.id} expects ${binding[1]} from package index`,
      );
    if (
      binding &&
      !readFileSync(
    ...
    if (
      JSON.stringify(Object.keys(guide.inputs).sort()) !==
      JSON.stringify(topLevel.sort())
    )
      throw Error(`Guide inputs drift: ${catalog.id}`);
    if (!binding)
      throw Error(`Missing JavaScript binding guide execution: ${catalog.id}`);
    const logs = [];
    const assertFiniteLogValue = (value, path = "result") => {
      if (value === undefined)
        throw Error(`Guide example logged undefined at ${catalog.id} ${path}`);
      if (typeof value === "number" && !Number.isFinite(value))
        throw Error(
          `Guide example logged non-finite number at ${catalog.id} ${path}`,
        );
      if (Array.isArray(value)) {
        value.forEach((item, index) =>
          assertFiniteLogValue(item, `${path}[${index}]`),
        );
        return;
      }
      if (value && typeof value === "object")
        Object.entries(value).forEach(([key, item]) =>
          assertFiniteLogValue(item, `${path}.${key}`),
        );
    };
    const capture = {
      log: (...values) => {
        const logged = values.length === 1 ? values[0] : values;
        assertFiniteLogValue(logged);
        logs.push(logged);
      },
    };
    Function(
      ...
      "console",
      `"use strict";\n${guide.code}`,
    )(packageBindings[binding[1]], capture);
    if (logs.length === 0)
      throw Error(`Guide example did not log: ${catalog.id}`);
    return {
      id: catalog.id,
      version: catalog.version,
      role: catalog.role,
      description: catalog.description,
      input: catalog.input_schema,
      query: catalog.query_schema ?? catalog.access_schema,
      queryOutput: catalog.query_output_schema ?? catalog.point_schema,
      output: catalog.output_schema,
      errors: catalog.errors ?? {},
      errorOrder: catalog.error_order ?? [],
      behavior: catalog.behavior ?? {},
      provenance: (catalog.provenance ?? []).map(({ candidate_id }) => ({
        candidate: candidate_id,
        sketch: candidate_id.split("#")[0],
      })),
      javascript: binding
        ? {
            exports: [binding[1]],
            module: `packages/javascript/src/${binding[0]}.js`,
            package: "@procedurals/javascript",
          }
        : null,
      guide: {
        ...guide,
        importLine: `import { ${binding[1]} } from "@procedurals/javascript";`,
        exampleResult: logs,
      },
      support: Object.entries(validation?.targets ?? {}).map(
        ([target, value]) => ({
          target,
...
const payload =
  JSON.stringify({ schemaVersion: 1, operations: entries }, null, 2) + "\n";
