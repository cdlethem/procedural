#!/usr/bin/env python3
"""Prepare existing quadrant vectors/access checks for ART; no native execution."""
import argparse
import json
import os
from pathlib import Path
from prepare_circle_placement_android_core import run, require_file, java_home, d8_path, tree_hashes
from run_quadrant_partition_java import ROOT, CATALOG, FIXTURE, CORE, NATIVE, sha, vector_source
...
def access_source():
    ...
    if source.count(marker) != 1:
        raise RuntimeError("quadrant access extraction marker changed")
    prefix = source[:source.index(marker)]
    for before, after in [("public final class QuadrantPartitionNative", "public final class QuadrantPartitionAndroidAccess"),
                          ("private QuadrantPartitionNative()", "private QuadrantPartitionAndroidAccess()"),
                          ("    private static volatile long sink;\n", "")]:
        if prefix.count(before) != 1:
            raise RuntimeError("quadrant access declaration changed")
...
def main():
    ...
    paths = [CATALOG, FIXTURE, CORE, NATIVE, Path(__file__).resolve(),
             ROOT / "tools/run_quadrant_partition_java.py", ROOT / "tools/prepare_circle_placement_android_core.py"]
    ...
    vectors = output / "QuadrantPartitionVectors.java"
    access = output / "QuadrantPartitionAndroidAccess.java"
    ...
    results = {}
    for name in ("QuadrantPartitionVectors", "QuadrantPartitionAndroidAccess"):
        results[name] = json.loads(run([java / "bin/java", "-cp", classes, "org.procedurals.layout." + name], timeout=60, env=env).stdout)
        if results[name].get("status") != "passed":
            raise RuntimeError("desktop preflight failed: " + name)
    expected_traces = sum("error" not in case and "selection_trace" in case for case in fixture["cases"])
    if results["QuadrantPartitionVectors"]["fixture_cases"] != len(fixture["cases"]) or results["QuadrantPartitionVectors"]["trace_cases"] != expected_traces:
    ...
    report = {"status": "passed", "scope": "Desktop quadrant fixtures/access and API33 dex preparation only; no ART/native support claim",
