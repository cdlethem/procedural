...
import {startLoopMarksServer} from './serve_loop_marks.mjs';
...
const server=await startLoopMarksServer(0);let browser;
const out=path.resolve(process.argv[2] || '.work/port-loop-native');
assert.ok(out.startsWith(path.join(root,'.work')+path.sep));await fs.mkdir(out);
const sources=['packages/javascript/src/internal/drawing-state.js','packages/javascript/src/cyclic-palette.js','packages/javascript/src/internal/fdlibm-hypot.js','packages/javascript/src/internal/p5-frame.js','packages/javascript/src/closed-spline.js','packages/javascript/examples/loop-marks/sketch.js','packages/javascript/examples/loop-marks/loop-marks.js','packages/javascript/examples/loop-marks/index.html','tools/serve_loop_marks.mjs','tools/run_p5_loop_marks.mjs'];
...
try {
 ...
 for(const key of ['t','c','m','0']){await page.keyboard.press(key);await page.waitForFunction(n=>Number(document.querySelector('#art').dataset.revision)===n,++rev);const hash=await capture(key==='0'?'reset':key);if(key==='0')assert.equal(hash,base);else assert.notEqual(hash,base);}
