#!/usr/bin/env python3
"""Compile the WarpMarks PDE and optionally run its bounded JAVA2D lifecycle probe."""
...
PDE = ROOT / 'packages/java-processing/examples/WarpMarks/WarpMarks.pde'
PROBE = ROOT / 'tests/native/WarpMarksProbe.java'
PLAN = ROOT / 'design/capabilities/warp-marks-native-plan.md'
IDS = ['baseline', 'strength64', 'zero', 'restored', 'sinusoidal', 'stripes', 'reset']
...
def main():
    ...
    try:
        jar, generated = output / 'procedurals-core-cp13.jar', output / 'WarpMarks.java'
        ...
        run([jdk / 'bin/java', '-Duser.home=' + str(home), '-cp', str(prep) + os.pathsep + prepath,
             'PreprocessSketch', PDE, generated, 'WarpMarks'])
        ...
        if args.native:
            ...
            run([sys.executable, lease, '--timeout', '120', '--', 'xvfb-run', '-a', jdk / 'bin/java',
                 ...
                 'WarpMarksProbe', native_out, jar], timeout=150)
            native = json.loads((native_out / 'native.json').read_text())
            if native.get('status') != 'passed' or native.get('frames') != 7 or native.get('keys') != 'wwwfp0s':
