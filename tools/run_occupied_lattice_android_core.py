#!/usr/bin/env python3
"""Run prepared occupied-lattice-paths vectors on a live API33 ART runtime; no drawing or APK install.
...
def verify_prepared(directory):
    ...
    if set(manifest['generated_sources_sha256']) != {'OccupiedLatticeVectors.java', 'OccupiedLatticeAndroidAccess.java'}:
...
def main():
    ...
    remote = '/data/local/tmp/procedurals-occupied-lattice-' + digest(dex)[:16] + '-' + str(time.monotonic_ns()) + '.dex'
    report = {'status': 'failed', 'scope': 'API33 ART pure occupied-lattice-paths vectors and ownership/access only; no renderer or performance claim.',
    ...
    try:
        ...
        expected_cases = prepared['desktop_preflight']['OccupiedLatticeVectors']['cases']
        expected_assertions = prepared['desktop_preflight']['OccupiedLatticeAndroidAccess']['assertions']
        if report['results']['OccupiedLatticeVectors'].get('cases') != expected_cases:
            raise RuntimeError('incomplete shared fixture execution')
        if report['results']['OccupiedLatticeAndroidAccess'].get('assertions') != expected_assertions:
            raise RuntimeError('ownership/access checks missing')
        if report['results']['OccupiedLatticeAndroidAccess'].get('native_ownership_access') is not True:
