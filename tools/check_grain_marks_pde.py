#!/usr/bin/env python3
"""Preprocess and compile the actual GrainMarks PDE with pinned Processing; no render."""
...
def installed_examples(jar: Path | None, report: Path | None, root_pde: Path,
    ...
    example = jar.parent.parent / "examples" / "GrainMarks"
    pde, composition = example / "GrainMarks.pde", example / "GrainComposition.java"
    if not pde.is_file() or not composition.is_file():
        raise RuntimeError("installed library layout lacks editable GrainMarks tabs")
    ...
    checks = ((pde, root_pde, "procedurals/examples/GrainMarks/GrainMarks.pde"),
              (composition, root_composition, "procedurals/examples/GrainMarks/GrainComposition.java"))
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / ".work/build/grain-marks-pde/report.json")
    ...
    build = ROOT / (".work/build/grain-marks-pde-installed" if library_jar else ".work/build/grain-marks-pde")
    if library_jar and build.exists():
        shutil.rmtree(build)
    build.mkdir(parents=True, exist_ok=True)
    (build / "home").mkdir(exist_ok=True)
    root_pde = ROOT / "packages/java-processing/examples/GrainMarks/GrainMarks.pde"
    root_composition = ROOT / "packages/java/examples/GrainMarks/GrainComposition.java"
    ...
    for item in inputs:
        if not item.is_file():
            raise FileNotFoundError("GrainMarks compile input missing: " + str(item))
    ...
    generated = build / "GrainMarks.java"
    run([home / "bin/java", "-Duser.home=" + str(build / "home"), "-cp",
         str(build) + os.pathsep + preprocess_classpath,
         "PreprocessSketch", pde, generated, "GrainMarks"], env=environment)
    ...
    if before != after:
        raise RuntimeError("GrainMarks sources changed during preprocessing/compilation")

    report = {
        "status": "passed",
        "scope": "Official Processing 4.5.6 GrainMarks PDE preprocessing and Java compilation only; no GUI lifecycle, renderer, image, or native support evidence was run.",
        "input_sha256_before": before,
        "input_sha256_after": after,
        "generated_java_sha256": digest(generated),
        "class_sha256": digest(build / "GrainMarks.class"),
