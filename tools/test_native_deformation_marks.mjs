...
const cases = {
  "pull-marks": [
    ...
    ["spokes-offset", { sourceMode: "spokes", pathCount: 20, jitter: 25, influenceCount: 2, centerX2: 135, centerY2: 500, radius2: 290, power2: .35 }],
  ],
  "projection-marks": [
    ["columns-one", { sourceMode: "columns", pathCount: 32, influenceCount: 1, centerX1: 320, centerY1: 320, radius1: 200, strength: 1 }],
    ["spokes-two", { sourceMode: "spokes", pathCount: 48, jitter: 12, influenceCount: 2, centerX2: 145, centerY2: 465, radius2: 250, strength: .9 }],
...
try {
  for (const [slug, variants] of Object.entries(cases)) {
    ...
    const initialState = await page.evaluate(name => window[name].snapshot(), exposed);
    assert.equal(initialState.sourceMode, "rows", `${slug} starts with the current Studio row source`);
    ...
    const exportedCorners = await page.evaluate(async encoded => {
    ...
    assert.deepEqual(exportedCorners, [0, 0, 0, 0], `${slug} exported PNG stays transparent at corners`);
    await page.reload();
    await page.waitForFunction(() => document.querySelector("#art")?.dataset.revision === "1");
