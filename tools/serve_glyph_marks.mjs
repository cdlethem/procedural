/** Serve the editable p5 GlyphMarks example locally using the pinned, repository-local runtime. */
...
export async function startGlyphMarksServer(port = 8794) {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      let relative;
      if (pathname === "/") { res.writeHead(302, { Location: "/packages/javascript/examples/glyph-marks/index.html" }); res.end(); return; }
      if (pathname === "/p5.js") relative = ".work/environments/p5js/node_modules/p5/lib/p5.min.js";
      else if (/^\/packages\/javascript\/(src\/(internal\/)?[a-z0-9-]+\.js|examples\/glyph-marks\/(index\.html|sketch\.js|glyph-marks\.js|assets\/(GlyphMarks\.ttf|FONT-LICENSE\.txt)))$/.test(pathname)) relative = pathname.slice(1);
      else { res.writeHead(404); res.end(); return; }
      const type = relative.endsWith(".html") ? "text/html; charset=utf-8"
        : relative.endsWith(".ttf") ? "font/ttf"
        : relative.endsWith(".txt") ? "text/plain; charset=utf-8"
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const server = await startGlyphMarksServer(Number(process.env.PORT ?? 8794));
  console.log(`Glyph marks: http://127.0.0.1:${server.address().port}/ — Ctrl-C to stop`);
