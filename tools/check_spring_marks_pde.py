#!/usr/bin/env python3
"""Compile SpringMarks and run its pure helper check; never launch Processing."""
...
def main()->int:
 ...
 helper=ROOT/'packages/java/examples/SpringMarks/SpringComposition.java'; pde=ROOT/'packages/java-processing/examples/SpringMarks/SpringMarks.pde'; native=ROOT/'tests/native/SpringCompositionNative.java'; bridge=ROOT/'tests/native/PreprocessSketch.java'
 ...
 prelibs=[runtime/'preprocessor'/n for n in NAMES]; pre=os.pathsep.join(map(str,[core,*prelibs])); generated=build/'SpringMarks.java'; report={'status':'failed','scope':'Official PDE compilation and pure continuing SpringComposition checks only; no Processing launch, renderer, pixel, or delivery claim.','inputs_before':before,'commands':commands}
 try:
  run([java/'javac','-cp',pre,'-d',build,bridge])
  run([java/'java','-Duser.home='+str(build/'home'),'-cp',str(build)+os.pathsep+pre,'PreprocessSketch',pde,generated,'SpringMarks'])
  cp=os.pathsep.join(map(str,[core,build]));run([java/'javac','--release','17','-cp',cp,'-d',build,*java_sources,helper,native,generated])
  run([java/'java','-Djava.awt.headless=true','-cp',cp,'SpringCompositionNative'])
