#!/usr/bin/env node
/** Selected six-study native evidence. Run through with_native_render_lock.py. */
...
const studies=[['cell-mosaic','Cells',72],['cell-echoes','Echoes',12],['stitched-paths','Stitches per path',105],['orbit-beads','Beads per orbit',130],['contour-terrain','Terrain scale',0.012],['contour-blobs','First level',0.85]];
...
const inputs=[fileURLToPath(import.meta.url),...await files('packages/javascript/src'),...await files('packages/javascript/src/internal'),...studies.flatMap(([s])=>[`packages/javascript/examples/${s}/README.md`,`packages/javascript/examples/${s}/index.html`,`packages/javascript/examples/${s}/sketch.js`]),...await files('apps/web/lib/adapters')];
...
const report={status:'failed',scope:'Six original design studies, p5 Canvas2D Chromium. Actual gallery adapter baseline/structural edit/palette transfer/reload and standalone example controls/reset/save. No corpus recreation or other-target claim.',input_sha256_before:await hashes(),studioBinding:JSON.parse(await readFile(path.join(root,'apps/web/lib/generated-gallery.json'),'utf8')).studioBinding,studies:[]};
...
try{
 ...
 for(const [slug,label,value] of studies){
  ...
  try{
   ...
   await page.reload({waitUntil:'networkidle'});await page.locator('[data-render-status="ready"], [data-render-status="error"]').waitFor();assert.equal(await page.locator('[data-render-status]').first().getAttribute('data-render-status'),'ready',await page.locator('[data-render-status]').first().textContent());assert.equal(await pixels(),baseline,'gallery reload resets exactly');
   ...
   await page.goto(`http://127.0.0.1:${server.address().port}/packages/javascript/examples/${slug}/index.html`);await page.waitForFunction(()=>Number(document.querySelector('#art')?.dataset.revision)>=1);
   ...
   await edit('t','native-structural-edit');await edit('c','native-palette-transfer');await edit('0','native-reset');assert.equal(await pixels(),native,'native reset exact');
   const rev=await page.locator('#art').getAttribute('data-revision');const download=page.waitForEvent('download');await page.locator('button[data-action="s"]').click();await(await download).saveAs(path.join(dir,'saved.png'));assert.equal(await pixels(),native);assert.equal(await page.locator('#art').getAttribute('data-revision'),rev);
   await page.reload();await page.waitForFunction(()=>Number(document.querySelector('#art')?.dataset.revision)>=1);assert.equal(await pixels(),native,'native reload exact');assert.deepEqual(errors,[]);
 ...
 report.input_sha256_after=await hashes();assert.deepEqual(report.input_sha256_after,report.input_sha256_before);report.status='passed';
