#!/usr/bin/env python3
"""Build the local Android PlacementMarks 0.3.0 starter and compile its ZIP."""
...
VERSION = "0.3.0"
OUTPUT = ROOT / ".work/dist/cp3/android-placement1"
CORE = ROOT / ".work/dist/cp3/java/procedurals-core-0.3.0.jar"
ADAPTER = ROOT / ".work/dist/cp2/android/procedurals-android-0.2.0.jar"
CORE_EVIDENCE = ROOT / "evidence/distribution/cp3-java.json"
ADAPTER_EVIDENCE = ROOT / "evidence/distribution/cp2-android.json"
EXAMPLE = ROOT / "packages/java-android/examples/PlacementMarks"
COMPOSITION = ROOT / "packages/java/examples/PlacementMarks/PlacementComposition.java"
GALLERY = ROOT / "packages/java-android/examples/FieldMarks/GalleryWriter.java"
STARTER_ROOT = "procedurals-placement-marks-android"
...
def transformed_gradle() -> tuple[str, list[dict[str, str]]]:
    ...
    replacements = (("namespace 'org.procedurals.fieldmarks'", "namespace 'org.procedurals.placementmarks'", "namespace"),
                    ("applicationId 'org.procedurals.fieldmarks'", "applicationId 'org.procedurals.placementmarks'", "applicationId"),
                    ("versionCode 1", "versionCode 3", "versionCode"),
                    ("versionName '0.1.0'", "versionName '0.3.0'", "versionName"))
    records = []
...
def starter_readme() -> str:
    return f"""# Procedurals Android PlacementMarks starter

This API-29+ project contains the editable PlacementMarks Activity and Renderer,
the shared PlacementComposition, and GalleryWriter. The portable core and Android
...
Seed and Budget change the proposal sequence; Source switches to authored radial
bands; Separation changes exclusion distance; Min/Max change radius bounds.
Seed/Budget/Min/Max are ignored for radial proposals. Motif and Palette reuse the
retained placement. Save PNG writes the displayed image to Pictures/Procedurals.
...
def stage_project(stage: Path, core: Path, adapter: Path) -> tuple[dict[str, str], list[dict[str, str]]]:
    write_project_base(stage, "ProceduralsPlacementMarks", "app")
    ...
    targets = {
        EXAMPLE / "PlacementMarksActivity.java": java / "org/procedurals/examples/placementmarks/PlacementMarksActivity.java",
        EXAMPLE / "PlacementMarksRenderer.java": java / "org/procedurals/examples/placementmarks/PlacementMarksRenderer.java",
        COMPOSITION: java / "org/procedurals/examples/placementmarks/PlacementComposition.java",
...
def main() -> int:
    ...
    try: output.relative_to((ROOT / ".work/dist/cp3").resolve())
    except ValueError as error: raise ValueError("output must remain under .work/dist/cp3") from error
    ...
    core = require_file(args.core_jar, "CP3 core JAR"); adapter = require_file(args.adapter_jar, "CP2 adapter JAR")
    ...
    inputs = [Path(__file__).resolve(), CORE_EVIDENCE, ADAPTER_EVIDENCE, core, adapter, processing, gradle,
              ...
              EXAMPLE / "PlacementMarksActivity.java", EXAMPLE / "PlacementMarksRenderer.java", COMPOSITION, GALLERY,
    ...
    zip_path = output / f"procedurals-placement-marks-android-{VERSION}.zip"; zip_tree(staged, zip_path)
    ...
    for source, target in {EXAMPLE / "PlacementMarksActivity.java": extracted / "app/src/main/java/org/procedurals/examples/placementmarks/PlacementMarksActivity.java", EXAMPLE / "PlacementMarksRenderer.java": extracted / "app/src/main/java/org/procedurals/examples/placementmarks/PlacementMarksRenderer.java", COMPOSITION: extracted / "app/src/main/java/org/procedurals/examples/placementmarks/PlacementComposition.java", GALLERY: extracted / "app/src/main/java/org/procedurals/examples/fieldmarks/GalleryWriter.java"}.items():
    ...
    report = {"status":"passed", "package_version":VERSION, "scope":"Android PlacementMarks starter packaging and extracted debug APK compilation only; no install, launch, render, or Android support acceptance", "input_sha256_before":before, "input_sha256_after":after, "artifacts":{"starter":{"path":display(zip_path),"sha256":digest(zip_path),"entries":entries},"core":{"path":display(core),"sha256":digest(core)},"adapter":{"path":display(adapter),"sha256":digest(adapter)}}, "starter_sources_byte_exact":bindings, "starter_transformations":transformations, "consumer":{"extracted_core_byte_exact":True,"extracted_adapter_byte_exact":True,"compiled_apk_sha256":digest(apk),"external_processing_core":{"path":display(external),"sha256":digest(external)},"external_sdk":display(sdk)}, "zip_exclusions":{"processing_runtime":True,"sdk":True,"debug_key":True,"build_output":True}, "environment":{"java_home":display(java),"gradle":display(gradle),"sdk_platform":33,"build_tools":"30.0.3"}}
