...
assert.ok(['pixel-grain','field-displacement','octave-noise','pixel-grain-reproduction'].includes(slug));
const studyDirectory=slug==='pixel-grain-reproduction'?'tests/native/pixel-grain-reproduction':`packages/javascript/examples/${slug}`;
...
const inputs=[fileURLToPath(import.meta.url),path.join(root,'tools/serve_survey_coverage_studies.mjs'),...await sourceFiles(path.join(root,'packages/javascript/src')),...await sourceFiles(path.join(root,studyDirectory)),path.join(root,'.work/environments/p5js/node_modules/p5/lib/p5.min.js')];
...
try{
 ...
 await page.goto(`http://127.0.0.1:${server.address().port}/${studyDirectory}/index.html`);await ready();
 const frames=[];
 ...
 const baseline=await capture('baseline'),edit=await action('t','strength-edit');assert.notEqual(edit.sha256,baseline.sha256);if(slug.startsWith('pixel-grain'))assert.equal(edit.geometry,baseline.geometry);
