#!/usr/bin/env python3
"""Run prepared placement vectors on a live API33 ART runtime; no drawing or APK install.
...
def verify_prepared(directory):
    ...
    for name, key in [('CirclePlacementsVectors.java','generated_source_sha256'),
                      ('CirclePlacementsAndroidAccess.java','generated_access_source_sha256')]:
        if digest(directory / name) != manifest[key]:
            raise RuntimeError('generated source changed: ' + name)
    if digest(directory / 'classes.dex') != manifest['dex_sha256']['classes.dex']:
...
def main():
    ...
    remote = '/data/local/tmp/procedurals-placement-' + digest(dex)[:16] + '-' + str(time.monotonic_ns()) + '.dex'
    report = {'status': 'failed', 'scope': 'API33 ART pure placement vectors and ownership/access only; no renderer or performance claim.',
    ...
    try:
        ...
        report['results'] = {}
        for name in ('CirclePlacementsVectors', 'CirclePlacementsAndroidAccess'):
            result = run('shell', 'env', 'CLASSPATH=' + remote, 'app_process', '/system/bin',
                         'org.procedurals.sampling.' + name)
        ...
        if report['results']['CirclePlacementsVectors'].get('fixture_cases') != 68:
            raise RuntimeError('incomplete shared fixture execution')
        if report['results']['CirclePlacementsAndroidAccess'].get('assertions', 0) <= 0:
