#!/usr/bin/env python3
"""Run prepared delaunay vectors on a live API33 ART runtime; no drawing or APK install.
...
def verify_prepared(directory):
    ...
    if set(manifest['generated_sources_sha256']) != {'Delaunay2DVectors.java', 'Delaunay2DAndroidAccess.java'}:
...
def main():
    ...
    remote = '/data/local/tmp/procedurals-delaunay-' + digest(dex)[:16] + '-' + str(time.monotonic_ns()) + '.dex'
    report = {'status': 'failed', 'scope': 'API33 ART pure delaunay vectors and ownership/access only; no renderer or performance claim.',
    ...
    try:
        ...
        if report['results']['Delaunay2DVectors'].get('fixture_cases') != 36:
            raise RuntimeError('incomplete shared fixture execution')
        if report['results']['Delaunay2DAndroidAccess'].get('assertions') != 126:
            raise RuntimeError('ownership/access checks missing')
        if report['results']['Delaunay2DAndroidAccess'].get('native_ownership_access') is not True:
