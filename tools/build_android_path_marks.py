#!/usr/bin/env python3
"""Stage or compile the Android PathMarks example; never install, launch, or render it."""
...
DEFAULT_STAGE=ROOT/'.work/examples/android-path-marks'
DEFAULT_APP='org.procedurals.pathmarks'
DEFAULT_ACTIVITY='org.procedurals.examples.pathmarks.PathMarksActivity'
...
def require(path: Path) -> Path:
    if not path.is_file(): raise FileNotFoundError('required PathMarks input is missing: '+str(path))
    return path.resolve()
...
def source_inputs(extra_sources: tuple[Path,...]=()) -> tuple[Path,...]:
    sources=(*sorted((ROOT/'packages/java/src/main/java').rglob('*.java')),
             *sorted((ROOT/'packages/java-android/src/main/java').rglob('*.java')),
             require(EXAMPLE/'PathMarksActivity.java'),require(EXAMPLE/'PathMarksRenderer.java'),
             require(COMPOSITION),require(GALLERY),*(require(Path(source)) for source in extra_sources))
    names=[path.name for path in sources]
    if len(names)!=len(set(names)): raise RuntimeError('duplicate staged Java filename')
    return sources
def template_inputs() -> tuple[Path,...]:
    return (*sorted(path for path in BOOTSTRAP.rglob('*') if path.is_file()),require(EXAMPLE/'AndroidManifest.xml'),
            require(EXAMPLE/'README.md'),require(CORE),require(KEYSTORE),require(SDK/'platforms/android-33/android.jar'),require(GRADLE),Path(__file__).resolve())
...
def prepare(stage: Path=DEFAULT_STAGE, application_id: str=DEFAULT_APP, activity_class: str=DEFAULT_ACTIVITY,
    ...
    sources=source_inputs(extra_sources);templates=template_inputs();before=hashes((*sources,*templates));stage.mkdir(parents=True,exist_ok=True)
    ...
    for source in sources:shutil.copyfile(source,java/source.name)
...
def build(stage: Path) -> Path:
    ...
    with (stage/'build.log').open('w') as log:subprocess.run([str(GRADLE),'--no-daemon','--console=plain',':app:assembleDebug'],cwd=stage,env=env,stdout=log,stderr=subprocess.STDOUT,check=True)
