#!/usr/bin/env python3
"""Run a registered, source-bound ReliefMarks native attempt under the render lock."""
import argparse
import fcntl
import json
import os
...
STAGE = ROOT / '.work/examples/r1-candidate1'
BUILD = STAGE / 'build'
LIBRARY = STAGE / 'ReliefMarks/code/procedurals.jar'
PROBE_BUILD = ROOT / '.work/build/r1-probe-root1'
PROBE = ROOT / 'tools/diagnostics/r1/ReliefMarksProbe.java'
KEYS = 'c0h0r0s'
IDS = ['baseline','palette','reset-colour','tall','reset-height','new-seed','final-reset']
...
def png(path):
    ...
    with Image.open(path) as source:
        ...
        if rgba.size != (960, 960) or rgba.getchannel('A').getextrema() != (255, 255):
            raise RuntimeError('invalid PNG dimensions/alpha: ' + path.name)
        if len(rgba.convert('RGB').getcolors(960 * 960) or []) < 2:
            raise RuntimeError('blank PNG: ' + path.name)
    return {'path': rel(path), 'sha256': sha(path), 'width': 960, 'height': 960, 'alpha_opaque': True}
...
def bindings():
    ...
    for path in [PROBE, Path(__file__), STAGE / 'result.json', HOME / 'bin/java',
                 ...
                 ROOT / 'design/capabilities/momito-recreation-acceptance.md']:
        bound[rel(path)] = sha(path)
    for name, expected in JARS.items():
        path = RUNTIME / name
        if sha(path) != expected:
            raise RuntimeError('Changed coherent P3D runtime: ' + name)
        bound[rel(path)] = expected
    bound[rel(ROOT / 'tools/diagnostics/cp7/run_profiles.py')] = sha(ROOT / 'tools/diagnostics/cp7/run_profiles.py')
    bound.update(classes(BUILD))
    bound.update(classes(PROBE_BUILD))
    if set(classes(PROBE_BUILD)) != {rel(PROBE_BUILD / 'ReliefMarksProbe.class')}:
...
def command(output):
    return ['xvfb-run', '-a', str(HOME / 'bin/java'), '-Xmx512m',
            ...
            '-cp', os.pathsep.join(map(str, [BUILD, PROBE_BUILD, LIBRARY, *[RUNTIME / name for name in JARS]])),
            'ReliefMarksProbe', str(output), str(LIBRARY)]


def validate_plan(path):
    plan = json.loads(path.read_text())
    for key, value in {'status': 'ready', 'owner': 'root', 'keys': KEYS,
                       'frame_ids': IDS, 'expected_frames': 7, 'timeout_seconds': 180}.items():
...
def validate_native(output):
    ...
    for key, value in {'status': 'passed', 'frames': 7, 'key_events': 7, 'keys': KEYS,
                       'renderer_class': 'processing.opengl.PGraphics3D'}.items():
    ...
    saved = list(output.glob('relief-marks.png'))
        ...
        try:
            ...
            with (output / 'stdout.log').open('x') as stdout, (output / 'stderr.log').open('x') as stderr:
                ...
                try:
                    ...
                    process.wait()
                    raise RuntimeError('ReliefMarks timed out')
            report['exit_code'] = exit_code
            report['stderr'] = (output / 'stderr.log').read_text()
            if exit_code != 0 or (report['stderr'] and not KNOWN_STDERR.fullmatch(report['stderr'])):
