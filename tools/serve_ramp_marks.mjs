/** Serve the editable p5 RampMarks example locally using the pinned, repository-local runtime. */
...
export async function startRampMarksServer(port=8767) {
  ...
  const server=http.createServer(async(req,res)=>{
    try {
      ...
      let relative;
      if(pathname==='/'){res.writeHead(302,{Location:'/packages/javascript/examples/ramp-marks/index.html'});res.end();return;}
      if(pathname==='/p5.js')relative='.work/environments/p5js/node_modules/p5/lib/p5.min.js';
      else if(/^\/packages\/javascript\/(src\/(internal\/)?[a-z0-9-]+\.js|examples\/ramp-marks\/(index\.html|sketch\.js|ramp-marks\.js))$/.test(pathname))relative=pathname.slice(1);
...
if(process.argv[1]&&path.resolve(process.argv[1])===fileURLToPath(import.meta.url)) {
  const server=await startRampMarksServer();
  console.log(`Ramp marks: http://127.0.0.1:${server.address().port}/ — Ctrl-C to stop`);
