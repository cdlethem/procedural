#!/usr/bin/env python3
"""Run prepared quadrant vectors on a live API33 ART runtime; no drawing or APK install.
...
def verify_prepared(directory):
    ...
    if set(manifest['generated_sources_sha256']) != {'QuadrantPartitionVectors.java', 'QuadrantPartitionAndroidAccess.java'}:
...
def main():
    ...
    remote = '/data/local/tmp/procedurals-quadrant-' + digest(dex)[:16] + '-' + str(time.monotonic_ns()) + '.dex'
    report = {'status': 'failed', 'scope': 'API33 ART pure quadrant vectors and ownership/access only; no renderer or performance claim.',
    ...
    try:
        ...
        if report['results']['QuadrantPartitionVectors'].get('fixture_cases') != 34:
            raise RuntimeError('incomplete shared fixture execution')
        if report['results']['QuadrantPartitionVectors'].get('trace_cases') != 11:
            raise RuntimeError('incomplete successful selection traces')
        if report['results']['QuadrantPartitionAndroidAccess'].get('assertions') != 27:
            raise RuntimeError('ownership/access checks missing')
        if report['results']['QuadrantPartitionAndroidAccess'].get('native_ownership_access') is not True:
