#!/usr/bin/env python3
"""Compile the current Java core and run the bounded MaskedPartitionMarks JAVA2D probe."""
...
IDS = ["baseline", "shifted", "restored", "local", "crop", "reset"]
KEYS = "nnmm0s"
...
def validate_native(native: dict, output: Path, jar: Path) -> dict:
    if native.get("status") != "passed" or native.get("frames") != len(IDS) or native.get("keys") != KEYS:
        raise RuntimeError("Incomplete MaskedPartitionMarks native sequence")
    records = native.get("frame_records")
    if not isinstance(records, list) or [item.get("id") for item in records] != IDS:
        raise RuntimeError("Unexpected MaskedPartitionMarks frame records")
    if native.get("core_code_source") != str(jar) or native.get("expected_jar") != str(jar):
        raise RuntimeError("MaskedPartitionMarks did not use the source-built core JAR")
    if native.get("renderer") != "processing.awt.PGraphicsJava2D":
        raise RuntimeError("MaskedPartitionMarks renderer was not PGraphicsJava2D")
    if any(native.get(key) != value for key, value in (("width", 720), ("height", 480), ("density", 1))):
        raise RuntimeError("MaskedPartitionMarks dimensions or density mismatch")
    names = [*IDS, "masked-partition-marks"]
    images = {}
    for name in names:
        path = output / (name + ".png")
        if not path.is_file():
            raise RuntimeError("Missing MaskedPartitionMarks image: " + name)
...
def main() -> None:
    ...
    pde = ROOT / "packages/java-processing/examples/MaskedPartitionMarks/MaskedPartitionMarks.pde"
    probe = ROOT / "tests/native/MaskedPartitionMarksProbe.java"
    plan = ROOT / "design/capabilities/masked-partition-marks-native-plan.md"
    ...
    adapters = [ROOT / ("packages/java-processing/src/main/java/org/procedurals/processing/" + name + ".java") for name in ("Java2DLayers", "Java2DRegions")]
    runtime_inputs = check_runtime()
    jdk_inputs = [jdk / name for name in ("bin/java", "bin/javac", "bin/jar", "release", "lib/modules")]
    inputs = [pde, probe, plan, bridge, lease, *adapters, Path(__file__), ROOT / "tools/check_field_marks_pde.py",
    ...
    commands = []
    report = {"status": "failed", "scope": "Candidate source-built core/JAR and actual MaskedPartitionMarks JAVA2D lifecycle; no distribution acceptance", "commands": commands, "input_sha256_before": before}
    ...
    try:
        ...
        candidate_jar, generated = output / "procedurals-core-candidate.jar", output / "MaskedPartitionMarks.java"
        ...
        result = invoke([java, "-Duser.home=" + str(home), "-cp", str(prep) + os.pathsep + prepath,
                         "PreprocessSketch", pde, generated, "MaskedPartitionMarks"])
        if result.returncode:
        ...
        result = invoke([javac, "--release", "8", "-cp", os.pathsep.join(map(str, [RUNTIME / "core-4.5.6.jar", candidate_jar])),
                         "-d", classes, *adapters, generated, probe])
        if result.returncode:
            raise RuntimeError("MaskedPartitionMarks/probe compilation failed")
        ...
        if args.native:
            ...
            command = [sys.executable, lease, "--timeout", "120", "--", "xvfb-run", "-a", java,
                       ...
                       "MaskedPartitionMarksProbe", native_out, candidate_jar]
            result = invoke(command, timeout=150)
            if result.returncode:
                raise RuntimeError("MaskedPartitionMarks native process failed")
            if result.stderr:
                raise RuntimeError("Unexpected native stderr diagnostics: " + result.stderr)
            native_path = native_out / "native.json"
            if not native_path.is_file():
                raise RuntimeError("MaskedPartitionMarks native process produced no native.json")
