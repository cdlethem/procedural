#!/usr/bin/env python3
"""Preprocess and compile the actual PlacementMarks PDE with pinned Processing; no renders."""
...
def installed_library(library: Path | None, report: Path | None) -> tuple[Path | None, Path | None]:
    ...
    if not contains_hash(payload.get('artifacts', {})):
        raise RuntimeError('distribution report does not bind the supplied installed JAR hash')
    return jar, evidence
...
def installed_examples(jar: Path | None, report: Path | None, root_pde: Path,
    ...
    extracted = jar.parent.parent / 'examples' / 'PlacementMarks'
    pde = extracted / 'PlacementMarks.pde'
    composition = extracted / 'PlacementComposition.java'
    if not pde.is_file() or not composition.is_file():
        raise RuntimeError('installed library layout lacks editable PlacementMarks tabs')
    ...
    starter_entries = payload.get('artifacts', {}).get('starter', {}).get('entries')
    if not isinstance(starter_entries, list):
        raise RuntimeError('distribution report lacks starter entries')
    entry_hashes = {entry.get('path'): entry.get('sha256') for entry in starter_entries if isinstance(entry, dict)}
    checks = ((pde, root_pde, 'procedurals/examples/PlacementMarks/PlacementMarks.pde'),
              (composition, root_composition, 'procedurals/examples/PlacementMarks/PlacementComposition.java'))
    for extracted_path, root_path, archive_path in checks:
        root_key = str(root_path.relative_to(ROOT))
        expected = before.get(root_key)
        if not isinstance(expected, str) or entry_hashes.get(archive_path) != expected:
            raise RuntimeError(f'distribution report does not bind {archive_path} to its root source')
...
def main() -> None:
    ...
    parser.add_argument('--output', type=Path,
                        default=ROOT / 'evidence/reproductions/cp3-java2d/pde-build.json')
    parser.add_argument('--library-jar', type=Path,
                        help='use this hash-bound installed Procedurals core JAR instead of compiling core sources')
    ...
    build = ROOT / ('.work/build/placement-marks-pde-installed' if library_jar else '.work/build/placement-marks-pde')
    build.mkdir(parents=True, exist_ok=True)
    (build / 'home').mkdir(exist_ok=True)
    root_pde = ROOT / 'packages/java-processing/examples/PlacementMarks/PlacementMarks.pde'
    root_composition = ROOT / 'packages/java/examples/PlacementMarks/PlacementComposition.java'
    ...
    for item in inputs:
        if not item.is_file():
            raise RuntimeError(f'missing PlacementMarks build input: {item}')
    ...
    generated = build / 'PlacementMarks.java'
    run([home / 'bin/java', f'-Duser.home={build / "home"}', '-cp', str(build) + os.pathsep + classpath,
         'PreprocessSketch', example_pde, generated, 'PlacementMarks'], env=environment)
    ...
    configuration_classpath = str(build) + os.pathsep + compile_classpath
    configuration_command = [home / 'bin/java', '-cp', configuration_classpath,
                             'PlacementMarksPdeConfiguration']
    if library_jar:
    ...
    if before != after:
        raise RuntimeError('PlacementMarks sources changed during preprocessing/compilation')
    report = {
        'status': 'passed',
        'scope': 'Official Processing 4.5.6 PlacementMarks PDE preprocessing and core-count configuration only; no GUI lifecycle or render was run.',
        'input_sha256': before,
        'generated_java_sha256': digest(generated),
        'class_sha256': digest(build / 'PlacementMarks.class'),
