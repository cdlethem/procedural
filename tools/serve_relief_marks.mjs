/** Serve the editable p5 ReliefMarks example locally using the pinned, repository-local runtime. */
...
export async function startReliefMarksServer(port = 8793) {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      let relative;
      if (pathname === "/") { res.writeHead(302, { Location: "/packages/javascript/examples/relief-marks/index.html" }); res.end(); return; }
      if (pathname === "/p5.js") relative = ".work/environments/p5js/node_modules/p5/lib/p5.min.js";
      else if (/^\/packages\/javascript\/(src\/(internal\/)?[a-z0-9-]+\.js|examples\/relief-marks\/(index\.html|sketch\.js|relief-marks\.js))$/.test(pathname)) relative = pathname.slice(1);
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const server = await startReliefMarksServer(Number(process.env.PORT ?? 8793));
  console.log(`Relief marks: http://127.0.0.1:${server.address().port}/ — Ctrl-C to stop`);
