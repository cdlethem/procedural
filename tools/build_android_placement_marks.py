#!/usr/bin/env python3
"""Stage or compile the Android PlacementMarks example; never install, launch, or render it."""
...
DEFAULT_STAGE = ROOT / ".work/examples/android-placement-marks"
DEFAULT_APP = "org.procedurals.placementmarks"
DEFAULT_ACTIVITY = "org.procedurals.examples.placementmarks.PlacementMarksActivity"
...
def require(path: Path) -> Path:
    if not path.is_file():
        raise FileNotFoundError("required PlacementMarks input is missing: " + str(path))
    return path.resolve()
...
def source_inputs(extra_sources: tuple[Path, ...] = ()) -> tuple[Path, ...]:
    sources = (*sorted((ROOT / "packages/java/src/main/java").rglob("*.java")),
               *sorted((ROOT / "packages/java-android/src/main/java").rglob("*.java")),
               require(EXAMPLE / "PlacementMarksActivity.java"),
               require(EXAMPLE / "PlacementMarksRenderer.java"),
...
def prepare(stage: Path = DEFAULT_STAGE, application_id: str = DEFAULT_APP,
    ...
    if before != hashes((*sources, *templates)):
        raise RuntimeError("PlacementMarks inputs changed during preparation")
    metadata = {
        "stage": display(stage),
