#!/usr/bin/env python3
"""Run prepared closed-spline vectors on a live API33 ART runtime; no drawing or APK install.
...
def verify_prepared(directory):
    ...
    if set(manifest['generated_sources_sha256']) != {'ClosedSplineVectors.java', 'ClosedSplineAndroidAccess.java'}:
...
def main():
    ...
    remote = '/data/local/tmp/procedurals-closed-spline-' + digest(dex)[:16] + '-' + str(time.monotonic_ns()) + '.dex'
    report = {'status': 'failed', 'scope': 'API33 ART pure closed-spline vectors and ownership/access only; no renderer or performance claim.',
    ...
    try:
        ...
        if report['results']['ClosedSplineVectors'].get('cases') != 14:
            raise RuntimeError('incomplete shared fixture execution')
        if report['results']['ClosedSplineAndroidAccess'].get('assertions') != 43:
            raise RuntimeError('ownership/access checks missing')
        if report['results']['ClosedSplineAndroidAccess'].get('native_ownership_access') is not True:
