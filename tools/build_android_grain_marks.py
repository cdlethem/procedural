#!/usr/bin/env python3
"""Stage or compile GrainMarks only; never install, launch, or render it."""
...
DEFAULT_STAGE=ROOT/".work/examples/android-grain-marks"
DEFAULT_APP="org.procedurals.grainmarks"
DEFAULT_ACTIVITY="org.procedurals.examples.grainmarks.GrainMarksActivity"
...
def require(path:Path)->Path:
    if not path.is_file():raise FileNotFoundError("required GrainMarks input is missing: "+str(path))
    return path.resolve()
...
def sources(extra:tuple[Path,...]=())->tuple[Path,...]:
    result=(*sorted((ROOT/"packages/java/src/main/java").rglob("*.java")),
            *sorted((ROOT/"packages/java-android/src/main/java").rglob("*.java")),
            require(EXAMPLE/"GrainMarksActivity.java"),require(EXAMPLE/"GrainMarksRenderer.java"),
...
def prepare(stage:Path=DEFAULT_STAGE,application_id:str=DEFAULT_APP,activity_class:str=DEFAULT_ACTIVITY,extra_sources:tuple[Path,...]=())->dict[str,object]:
    ...
    if before!=hashes((*java,*template)):raise RuntimeError("GrainMarks inputs changed during preparation")
