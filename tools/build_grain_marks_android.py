#!/usr/bin/env python3
"""Build the local Android GrainMarks 0.5.0 starter and compile its ZIP."""
...
VERSION = "0.5.0"
OUTPUT = ROOT / ".work/dist/cp5/android-grain1"
CORE = ROOT / ".work/dist/cp5/java/procedurals-core-0.5.0.jar"
ADAPTER = ROOT / ".work/dist/cp2/android/procedurals-android-0.2.0.jar"
CORE_EVIDENCE = ROOT / "evidence/distribution/cp5-java.json"
ADAPTER_EVIDENCE = ROOT / "evidence/distribution/cp2-android.json"
EXAMPLE = ROOT / "packages/java-android/examples/GrainMarks"
COMPOSITION = ROOT / "packages/java/examples/GrainMarks/GrainComposition.java"
GALLERY = ROOT / "packages/java-android/examples/FieldMarks/GalleryWriter.java"
STARTER_ROOT = "procedurals-grain-marks-android"
...
def transformed_gradle() -> tuple[str, list[dict[str, str]]]:
    ...
    replacements = (("namespace 'org.procedurals.fieldmarks'", "namespace 'org.procedurals.grainmarks'", "namespace"),
                    ("applicationId 'org.procedurals.fieldmarks'", "applicationId 'org.procedurals.grainmarks'", "applicationId"),
                    ("versionCode 1", "versionCode 5", "versionCode"),
                    ("versionName '0.1.0'", "versionName '0.5.0'", "versionName"))
    records = []
...
def starter_readme() -> str:
    return f"""# Procedurals Android GrainMarks starter

This API-29+ project contains the editable GrainMarks Activity and Renderer,
the shared GrainComposition, and GalleryWriter. The portable core and Android
...
Seed, Density, Distribution and Cells rebuild the retained triangle points. Reset
rebuilds the original configuration even when already at defaults. Motif and Palette
reuse the retained points. Save PNG writes the displayed image
...
def stage_project(stage: Path, core: Path, adapter: Path) -> tuple[dict[str, str], list[dict[str, str]]]:
    write_project_base(stage, "ProceduralsGrainMarks", "app")
    ...
    targets = {
        EXAMPLE / "GrainMarksActivity.java": java / "org/procedurals/examples/grainmarks/GrainMarksActivity.java",
        EXAMPLE / "GrainMarksRenderer.java": java / "org/procedurals/examples/grainmarks/GrainMarksRenderer.java",
        COMPOSITION: java / "org/procedurals/examples/grainmarks/GrainComposition.java",
...
def main() -> int:
    ...
    try: output.relative_to((ROOT / ".work/dist/cp5").resolve())
    except ValueError as error: raise ValueError("output must remain under .work/dist/cp5") from error
    ...
    core = require_file(args.core_jar, "CP5 core JAR"); adapter = require_file(args.adapter_jar, "CP2 adapter JAR")
    ...
    inputs = [Path(__file__).resolve(), CORE_EVIDENCE, ADAPTER_EVIDENCE, core, adapter, processing, gradle,
              ...
              EXAMPLE / "GrainMarksActivity.java", EXAMPLE / "GrainMarksRenderer.java", COMPOSITION, GALLERY,
    ...
    zip_path = output / f"procedurals-grain-marks-android-{VERSION}.zip"; zip_tree(staged, zip_path)
    ...
    for source, target in {EXAMPLE / "GrainMarksActivity.java": extracted / "app/src/main/java/org/procedurals/examples/grainmarks/GrainMarksActivity.java", EXAMPLE / "GrainMarksRenderer.java": extracted / "app/src/main/java/org/procedurals/examples/grainmarks/GrainMarksRenderer.java", COMPOSITION: extracted / "app/src/main/java/org/procedurals/examples/grainmarks/GrainComposition.java", GALLERY: extracted / "app/src/main/java/org/procedurals/examples/fieldmarks/GalleryWriter.java"}.items():
    ...
    report = {"status":"passed", "package_version":VERSION, "scope":"Android GrainMarks starter packaging and extracted debug APK compilation only; no install, launch, render, or Android support acceptance", "input_sha256_before":before, "input_sha256_after":after, "artifacts":{"starter":{"path":display(zip_path),"sha256":digest(zip_path),"entries":entries},"core":{"path":display(core),"sha256":digest(core)},"adapter":{"path":display(adapter),"sha256":digest(adapter)}}, "starter_sources_byte_exact":bindings, "starter_transformations":transformations, "consumer":{"extracted_core_byte_exact":True,"extracted_adapter_byte_exact":True,"compiled_apk_sha256":digest(apk),"external_processing_core":{"path":display(external),"sha256":digest(external)},"external_sdk":display(sdk)}, "zip_exclusions":{"processing_runtime":True,"sdk":True,"debug_key":True,"build_output":True}, "environment":{"java_home":display(java),"gradle":display(gradle),"sdk_platform":33,"build_tools":"30.0.3"}}
