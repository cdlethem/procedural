#!/usr/bin/env python3
"""Build the local Android BranchMarks 0.6.0 starter and compile its ZIP."""
...
VERSION = "0.6.0"
OUTPUT = ROOT / ".work/dist/cp6/android-branch1"
CORE = ROOT / ".work/dist/cp6/java/procedurals-core-0.6.0.jar"
ADAPTER = ROOT / ".work/dist/cp2/android/procedurals-android-0.2.0.jar"
CORE_EVIDENCE = ROOT / "evidence/distribution/cp6-java.json"
...
STARTER_ROOT = "procedurals-branch-marks-android"
...
def transformed_gradle() -> tuple[str, list[dict[str, str]]]:
    ...
    replacements = (("namespace 'org.procedurals.fieldmarks'", "namespace 'org.procedurals.branchmarks'", "namespace"),
                    ("applicationId 'org.procedurals.fieldmarks'", "applicationId 'org.procedurals.branchmarks'", "applicationId"),
                    ("versionCode 1", "versionCode 6", "versionCode"),
                    ("versionName '0.1.0'", "versionName '0.6.0'", "versionName"))
    records = []
...
def starter_readme() -> str:
    return f"""# Procedurals Android BranchMarks starter

This API-29+ project contains the editable BranchMarks Activity and Renderer,
the shared BranchComposition, and GalleryWriter. The portable core and Android
...
Seed, More generations, Narrow, Wide, Binary and Forest rebuild the retained trees.
Reset rebuilds the original configuration even when already at defaults. Taper and
Palette reuse retained geometry and ancestry. Save PNG writes the displayed image
...
def stage_project(stage: Path, core: Path, adapter: Path) -> tuple[dict[str, str], list[dict[str, str]]]:
    write_project_base(stage, "ProceduralsBranchMarks", "app")
    ...
    targets = {
        EXAMPLE / "BranchMarksActivity.java": java / "org/procedurals/examples/branchmarks/BranchMarksActivity.java",
        EXAMPLE / "BranchMarksRenderer.java": java / "org/procedurals/examples/branchmarks/BranchMarksRenderer.java",
        COMPOSITION: java / "org/procedurals/examples/branchmarks/BranchComposition.java",
...
def main() -> int:
    ...
    try: output.relative_to((ROOT / ".work/dist/cp6").resolve())
    except ValueError as error: raise ValueError("output must remain under .work/dist/cp6") from error
    ...
    core = require_file(args.core_jar, "CP6 core JAR"); adapter = require_file(args.adapter_jar, "CP2 adapter JAR")
    ...
    native_paths = [ROOT / 'evidence/conformance/branch-android-native-root-review.json',
                    ROOT / 'evidence/conformance/android-branch-marks.json',
                    ROOT / 'evidence/conformance/branch-tree-android.json',
                    ROOT / 'evidence/conformance/branch-android-readme-review.json']
    native_review = json.loads(native_paths[0].read_text())
    if (native_review.get('status'), native_review.get('owner'), native_review.get('reviewer')) != ('accepted','root','root'):
        raise RuntimeError('native review not accepted by root')
    for group in ('implementation_sha256', 'evidence_sha256'):
        for name, expected in native_review[group].items():
            if digest(ROOT / name) != expected: raise RuntimeError('native binding changed: ' + name)
            native_paths.append(ROOT / name)
    doc = json.loads((ROOT / 'evidence/conformance/branch-android-readme-review.json').read_text())
    if doc.get('status') != 'accepted' or doc.get('owner') != 'root' or doc.get('reviewer') != 'root' or digest(EXAMPLE / 'README.md') != doc['after_sha256']:
        raise RuntimeError('documentation delta is not reviewed')
    art = json.loads((ROOT / 'evidence/conformance/branch-tree-android.json').read_text())
    classes = {name: value for name, value in art['preparation']['generated_classes_sha256'].items()
               if name.startswith('org/procedurals/topology/BranchTree2D')}
    if not classes: raise RuntimeError('ART branch classes missing')
    with zipfile.ZipFile(core) as archive:
        for name, expected in classes.items():
            if hashlib.sha256(archive.read(name)).hexdigest() != expected:
                raise RuntimeError('packaged branch class differs from ART: ' + name)
    inputs = [*native_paths, Path(__file__).resolve(), CORE_EVIDENCE, ADAPTER_EVIDENCE, core, adapter, processing, gradle,
              ...
              EXAMPLE / "BranchMarksActivity.java", EXAMPLE / "BranchMarksRenderer.java", COMPOSITION, GALLERY,
    ...
    zip_path = output / f"procedurals-branch-marks-android-{VERSION}.zip"; zip_tree(staged, zip_path)
    ...
    for source, target in {EXAMPLE / "BranchMarksActivity.java": extracted / "app/src/main/java/org/procedurals/examples/branchmarks/BranchMarksActivity.java", EXAMPLE / "BranchMarksRenderer.java": extracted / "app/src/main/java/org/procedurals/examples/branchmarks/BranchMarksRenderer.java", COMPOSITION: extracted / "app/src/main/java/org/procedurals/examples/branchmarks/BranchComposition.java", GALLERY: extracted / "app/src/main/java/org/procedurals/examples/fieldmarks/GalleryWriter.java"}.items():
    ...
    report = {"status":"passed", "package_version":VERSION, "scope":"Android BranchMarks starter packaging and extracted debug APK compilation only; no install, launch, render, or Android support acceptance", "input_sha256_before":before, "input_sha256_after":after, "artifacts":{"starter":{"path":display(zip_path),"sha256":digest(zip_path),"entries":entries},"core":{"path":display(core),"sha256":digest(core)},"adapter":{"path":display(adapter),"sha256":digest(adapter)}}, "art_branch_classes_byte_exact":classes, "starter_sources_byte_exact":bindings, "starter_transformations":transformations, "consumer":{"extracted_core_byte_exact":True,"extracted_adapter_byte_exact":True,"compiled_apk_sha256":digest(apk),"external_processing_core":{"path":display(external),"sha256":digest(external)},"external_sdk":display(sdk)}, "zip_exclusions":{"processing_runtime":True,"sdk":True,"debug_key":True,"build_output":True}, "environment":{"java_home":display(java),"gradle":display(gradle),"sdk_platform":33,"build_tools":"30.0.3"}}
