#!/usr/bin/env python3
"""Run prepared raster-remap vectors on a live API33 ART runtime; no drawing or APK install.
...
def verify_prepared(directory):
    ...
    if set(manifest['generated_sources_sha256']) != {'V.java', 'RasterRemapAndroidAccess.java'}:
...
def main():
    ...
    remote = '/data/local/tmp/procedurals-raster-remap-' + digest(dex)[:16] + '-' + str(time.monotonic_ns()) + '.dex'
    report = {'status': 'failed', 'scope': 'API33 ART pure raster-remap vectors and ownership/access only; no renderer or performance claim.',
    ...
    try:
        ...
        if report['results']['V'].get('cases') != 13:
            raise RuntimeError('incomplete shared fixture execution')
        if report['results']['RasterRemapAndroidAccess'].get('assertions') != 23:
            raise RuntimeError('ownership/access checks missing')
        if report['results']['RasterRemapAndroidAccess'].get('native_ownership_access') is not True:
