#!/usr/bin/env python3
"""Build a CP11 candidate core and compile its editable staged sketch; no rendering."""
...
def main():
    ...
    review_path = ROOT / 'evidence/distribution/cp10-review.json'
    review = json.loads(review_path.read_text())
    core_review_path = ROOT / 'evidence/conformance/occupied-lattice-java-root-review.json'
    ...
    source = ROOT / 'packages/java/src/main/java/org/procedurals/paths/OccupiedLatticePaths2D.java'
    if sha(source) != core_review['core_sha256']:
        raise RuntimeError('Accepted OccupiedLatticePaths2D source changed')
    helper = ROOT / 'packages/java/examples/LatticeMarks/LatticeComposition.java'
    pde = ROOT / 'packages/java-processing/examples/LatticeMarks/LatticeMarks.pde'
    ...
    try:
        ...
        with zipfile.ZipFile(io.BytesIO(inherited)) as old, zipfile.ZipFile(library, 'x') as new:
            ...
            if not additions:
                raise RuntimeError('No OccupiedLatticePaths2D classes compiled')
            for path in additions:
                name = path.relative_to(addition).as_posix()
                if name in old_names or not name.startswith('org/procedurals/paths/OccupiedLatticePaths2D'):
        ...
        generated = build / 'LatticeMarks.java'
        run([jdk / 'bin/java', '-Duser.home=' + str(output / 'home'), '-cp', str(build) + os.pathsep + pre,
             'PreprocessSketch', sketch / pde.name, generated, 'LatticeMarks'])
        ...
        if list((build / 'org/procedurals/paths').glob('*.class')):
