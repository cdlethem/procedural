#!/usr/bin/env python3
"""Prepare existing stop-ramp vectors/access checks for ART; no native execution."""
import argparse
import json
import os
from pathlib import Path
from prepare_circle_placement_android_core import run, require_file, java_home, d8_path, tree_hashes
from run_stop_ramp_java import ROOT, CATALOG, FIXTURE, CORE, NATIVE, digest as sha, vector_source
...
def access_source():
    ...
    if source.count(marker) != 1:
        raise RuntimeError("stop-ramp access extraction marker changed")
    prefix = source[:source.index(marker)]
    if prefix.count("public final class StopRampNative") != 1:
        raise RuntimeError("stop-ramp access declaration changed")
    prefix = prefix.replace("public final class StopRampNative", "public final class StopRampAndroidAccess", 1)
...
def main():
    ...
    paths = [CATALOG, FIXTURE, CORE, NATIVE, Path(__file__).resolve(),
             ROOT / "tools/run_stop_ramp_java.py", ROOT / "tools/prepare_circle_placement_android_core.py"]
    ...
    vectors = output / "StopRampVectors.java"
    access = output / "StopRampAndroidAccess.java"
    ...
    for name in ("StopRampVectors", "StopRampAndroidAccess"):
        results[name] = json.loads(run([java / "bin/java", "-cp", classes, "org.procedurals.color." + name], timeout=60, env=env).stdout)
        if results[name].get("status") != "passed":
            raise RuntimeError("desktop preflight failed: " + name)
    if results["StopRampVectors"]["fixture_cases"] != len(fixture["cases"]):
    ...
    report = {"status": "passed", "scope": "Desktop stop-ramp fixtures/access and API33 dex preparation only; no ART/native support claim",
