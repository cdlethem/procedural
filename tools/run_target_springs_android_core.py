#!/usr/bin/env python3
"""Run prepared target-springs vectors on a live API33 ART runtime; no drawing or APK install.
...
def verify_prepared(directory):
    ...
    if set(manifest['generated_sources_sha256']) != {'TargetSpringsVectors.java', 'TargetSpringsAndroidAccess.java'}:
...
def main():
    ...
    remote = '/data/local/tmp/procedurals-target-springs-' + digest(dex)[:16] + '-' + str(time.monotonic_ns()) + '.dex'
    report = {'status': 'failed', 'scope': 'API33 ART pure target-springs vectors and ownership/access only; no renderer or performance claim.',
    ...
    try:
        ...
        if report['results']['TargetSpringsVectors'].get('fixture_cases') != 44:
            raise RuntimeError('incomplete shared fixture execution')
        if report['results']['TargetSpringsAndroidAccess'].get('assertions') != 1599:
            raise RuntimeError('ownership/access checks missing')
        if report['results']['TargetSpringsAndroidAccess'].get('native_ownership_access') is not True:
