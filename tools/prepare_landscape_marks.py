#!/usr/bin/env python3
"""Build a R3 unchanged core and compile its editable staged sketch; no rendering."""
...
def main():
    ...
    review_path = ROOT / 'evidence/distribution/r2-review.json'
    ...
    if sha(archive) != review['final_archive']['sha256']:
        raise RuntimeError('Accepted Java0.13 archive changed')
    helper = ROOT / 'packages/java/examples/LandscapeMarks/LandscapeComposition.java'
    pde = ROOT / 'packages/java-processing/examples/LandscapeMarks/LandscapeMarks.pde'
    ...
    try:
        ...
        generated = build / 'LandscapeMarks.java'
        run([jdk / 'bin/java', '-Duser.home=' + str(output / 'home'), '-cp', str(build) + os.pathsep + pre,
             'PreprocessSketch', sketch / pde.name, generated, 'LandscapeMarks'])
