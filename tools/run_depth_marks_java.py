#!/usr/bin/env python3
"""Compile the current Java core and run the bounded DepthMarks P3D probe."""
...
IDS = ["baseline", "depth_changed", "recolored", "mesh", "reset"]
KEYS = "zcm0s"
...
def validate_native(native: dict, output: Path, jar: Path) -> dict:
    if native.get("status") != "passed" or native.get("frames") != len(IDS) or native.get("keys") != KEYS:
        raise RuntimeError("Incomplete DepthMarks native sequence")
    records = native.get("frame_records")
    if not isinstance(records, list) or [item.get("id") for item in records] != IDS:
        raise RuntimeError("Unexpected DepthMarks frame records")
    if native.get("core_code_source") != str(jar) or native.get("expected_jar") != str(jar):
        raise RuntimeError("DepthMarks did not use the source-built core JAR")
    if native.get("renderer") != "processing.opengl.PGraphics3D":
        raise RuntimeError("DepthMarks renderer was not PGraphics3D")
    if any(native.get(key) != value for key, value in (("width", 640), ("height", 640), ("density", 1))):
        raise RuntimeError("DepthMarks dimensions or density mismatch")
    names = [*IDS, "depth-marks"]
    images = {}
    for name in names:
        path = output / (name + ".png")
        if not path.is_file():
            raise RuntimeError("Missing DepthMarks image: " + name)
...
def main() -> None:
    ...
    pde = ROOT / "packages/java-processing/examples/DepthMarks/DepthMarks.pde"
    probe = ROOT / "tests/native/DepthMarksProbe.java"
    plan = ROOT / "design/capabilities/depth-marks-native-plan.md"
    ...
    commands = []
    report = {"status": "failed", "scope": "Candidate source-built core/JAR and actual DepthMarks P3D lifecycle; no distribution acceptance", "commands": commands, "input_sha256_before": before}
    ...
    try:
        ...
        candidate_jar, generated = output / "procedurals-core-candidate.jar", output / "DepthMarks.java"
        ...
        result = invoke([java, "-Duser.home=" + str(home), "-cp", str(prep) + os.pathsep + prepath,
                         "PreprocessSketch", pde, generated, "DepthMarks"])
        if result.returncode:
        ...
        if result.returncode:
            raise RuntimeError("DepthMarks/probe compilation failed")
        ...
        if args.native:
            ...
            command = [sys.executable, lease, "--timeout", "120", "--", "xvfb-run", "-a", java,
                       ...
                       "DepthMarksProbe", native_out, candidate_jar]
            result = invoke(command, timeout=150)
            if result.returncode:
                raise RuntimeError("DepthMarks native process failed")
            ...
            if not native_path.is_file():
                raise RuntimeError("DepthMarks native process produced no native.json")
