#!/usr/bin/env python3
"""Build a CP10 candidate core and compile its editable staged sketch; no rendering."""
...
def main():
    ...
    review_path = ROOT / 'evidence/distribution/cp9-review.json'
    review = json.loads(review_path.read_text())
    core_review_path = ROOT / 'evidence/conformance/target-springs-java-root-review.json'
    ...
    if sha(archive) != review['final_archive']['sha256']:
        raise RuntimeError('Accepted CP9 archive changed')
    source = ROOT / 'packages/java/src/main/java/org/procedurals/motion/TargetSprings2D.java'
    if sha(source) != core_review['implementation_sha256'][str(source.relative_to(ROOT))]:
        raise RuntimeError('Accepted TargetSprings2D source changed')
    helper = ROOT / 'packages/java/examples/SpringMarks/SpringComposition.java'
    pde = ROOT / 'packages/java-processing/examples/SpringMarks/SpringMarks.pde'
    native = ROOT / 'tests/native/SpringCompositionNative.java'
    ...
    try:
        ...
        with zipfile.ZipFile(io.BytesIO(inherited)) as old, zipfile.ZipFile(library, 'x') as new:
            ...
            if not additions:
                raise RuntimeError('No TargetSprings2D classes compiled')
            for path in additions:
                name = path.relative_to(addition).as_posix()
                if name in old_names or not name.startswith('org/procedurals/motion/TargetSprings2D'):
        ...
        generated = build / 'SpringMarks.java'
        run([jdk / 'bin/java', '-Duser.home=' + str(output / 'home'), '-cp', str(build) + os.pathsep + pre,
             'PreprocessSketch', sketch / pde.name, generated, 'SpringMarks'])
        ...
        run([jdk / 'bin/java', '-Djava.awt.headless=true', '-cp', cp, 'SpringCompositionNative'])
        if list((build / 'org/procedurals/motion').glob('*.class')):
