#!/usr/bin/env python3
"""Run a registered, source-bound FacetMarks native attempt under the render lock."""
import argparse
import fcntl
import json
import os
...
STAGE = ROOT / '.work/examples/cp9-candidate1'
BUILD = STAGE / 'build'
LIBRARY = STAGE / 'FacetMarks/code/procedurals.jar'
PROBE_BUILD = ROOT / '.work/build/cp9-probe-root1'
PROBE = ROOT / 'tools/diagnostics/cp9/FacetMarksProbe.java'
KEYS = 'mm0c0p0n0r0xmmnc0s'
IDS = ['baseline', 'wire', 'grain', 'reset-mode', 'colour', 'reset-colour', 'sites',
       'reset-sites', 'fine', 'reset-fine', 'new-seed', 'reset-seed', 'cells',
       'cells-wire', 'cells-grain', 'cells-fine', 'cells-colour', 'final-reset']
...
def bindings():
    ...
    for path in [PROBE, Path(__file__), STAGE / 'result.json', HOME / 'bin/java',
                 ...
                 ROOT / 'design/capabilities/cp9-facet-marks-acceptance.md']:
        bound[rel(path)] = sha(path)
    bound.update(classes(BUILD))
    bound.update(classes(PROBE_BUILD))
    if set(classes(PROBE_BUILD)) != {rel(PROBE_BUILD / 'FacetMarksProbe.class')}:
...
def command(output):
    return ['xvfb-run', '-a', str(HOME / 'bin/java'), '-Xmx512m',
            ...
            'FacetMarksProbe', str(output), str(LIBRARY)]


def validate_plan(path):
    plan = json.loads(path.read_text())
    for key, value in {'status': 'ready', 'owner': 'root', 'keys': KEYS,
                       'frame_ids': IDS, 'expected_frames': 18, 'timeout_seconds': 180}.items():
...
def validate_native(output):
    ...
    for key, value in {'status': 'passed', 'frames': 18, 'key_events': 18, 'keys': KEYS,
    ...
    for i, r in enumerate(native['states']):
        mode = 1 if i in (1, 13) else 2 if i in (2, 14, 15, 16) else 0
        for key, expected in {'triangles': r['faces'] if mode == 0 else 0,
                              'lines': r['edges'] if mode == 1 else 0,
                              'points': r['grain'] if mode == 2 else 0,
                              'ellipses': r['vertices'] if i == 6 else 0,
                              'retained': i in (1, 2, 4, 6, 13, 14, 16)}.items():
            if r[key] != expected:
                raise RuntimeError('State count/retention differs: ' + r['id'])
    ...
    saved = list(output.glob('facet-marks-*.png'))
        ...
        try:
            ...
            with (output / 'stdout.log').open('x') as stdout, (output / 'stderr.log').open('x') as stderr:
                ...
                try:
                    ...
                    process.wait()
                    raise RuntimeError('FacetMarks timed out')
