#!/usr/bin/env node
/** Build the local CP4 JavaScript package and an installable RegionMarks starter. */
...
const REGION = join(PACKAGE, "examples/region-marks");
const OUTPUT_ROOT = join(ROOT, ".work/dist/cp4");
const VERSION = "0.4.0";
const NAME = "@procedurals/javascript";
const NOTICES = [join(ROOT, "LICENSE"), join(ROOT, "THIRD_PARTY_NOTICES.md")];
const BASE = join(ROOT, "fixtures/operations/seeded-quadrant-partition.json");
...
function required(path) { if(!existsSync(path)) throw new Error("Missing CP4 input: "+display(path)); return path; }
...
`; }
function starterPackage(tarball) { return JSON.stringify({name:"procedurals-region-marks-starter",private:true,version:VERSION,type:"module",scripts:{start:"node server.mjs"},dependencies:{[NAME]:`file:vendor/${tarball}`,p5:"2.3.2"}},null,2)+"\n"; }
function transformSources(starter) {
  const index=readFileSync(join(REGION,"index.html"),"utf8");
  const model=readFileSync(join(REGION,"region-marks.js"),"utf8");
  const sketch=readFileSync(join(REGION,"sketch.js"),"utf8");
  const importMap=`<script type="importmap">{"imports":{"${NAME}":"/node_modules/${NAME}/src/index.js","${NAME}/":"/node_modules/${NAME}/"}}</script>`;
  if(model.split('"../../src/quadrant-partition.js"').length!==2||index.split("</head>").length!==2)throw new Error("Unexpected RegionMarks import template");
  const generatedIndex=index.replace("</head>",`${importMap}</head>`);
  if(model.split('"../../src/regular-grid.js"').length!==2)throw new Error("Unexpected grid import");
  const generatedModel=model.replace('"../../src/quadrant-partition.js"',`"${NAME}"`).replace('"../../src/regular-grid.js"',`"${NAME}"`);
  if(generatedModel!==model.replace('"../../src/quadrant-partition.js"',`"${NAME}"`).replace('"../../src/regular-grid.js"',`"${NAME}"`)||sketch!==readFileSync(join(REGION,"sketch.js"),"utf8"))throw new Error("Unexpected starter rewrite");
  write(join(starter,"index.html"),generatedIndex); write(join(starter,"region-marks.js"),generatedModel); write(join(starter,"sketch.js"),sketch);
  return {originals:{index,model,sketch},generated:{index:generatedIndex,model:generatedModel,sketch},rewrites:{"index.html":{inserted:importMap},"region-marks.js":{from:["../../src/quadrant-partition.js","../../src/regular-grid.js"],to:NAME},"sketch.js":{byte_identical:true}}};
}
...
const cases=[createSeededRegions(42,100,.5),createSeededRegions(42,100,1),createSeededRegions(42,200,1),createSeededRegions(43,200,1),createAuthoredRegions()];
const expected=[301,301,601,601,11], fingerprints=[];
for(let c=0;c<cases.length;c++) {
 const regions=cases[c], ids=new Set(), values=[], bounds=new Float64Array(4), point=new Float64Array(2);
 if(regions.size!==expected[c])throw new Error('region count '+c);
 for(let i=0;i<regions.size;i++) {
  const id=regions.idAt(i); if(ids.has(id))throw new Error('duplicate cell');ids.add(id);
  regions.boundsInto(i,bounds);if(!bounds.every(Number.isFinite)||bounds[2]<bounds[0]||bounds[3]<bounds[1])throw new Error('bounds');
  values.push(id,...bounds);
  for(let j=0;j<9;j++){regions.markInto(j,bounds,point);if(!point.every(Number.isFinite)||point[0]<bounds[0]||point[0]>bounds[2]||point[1]<bounds[1]||point[1]>bounds[3])throw new Error('mark outside cell');values.push(...point);}
 }
 fingerprints.push((await import('node:crypto')).createHash('sha256').update(JSON.stringify(values)).digest('hex'));
}
if(new Set(fingerprints).size!==5)throw new Error('geometry edits unchanged');
...
console.log(JSON.stringify({counts:cases.map(c=>c.size),geometry_fingerprints:fingerprints,previous_exports:true,resolved,p5_version:p5.version}));`);
  const result=json(run("node",[".consumer-smoke.mjs"],{cwd:starter,timeout:180000}).stdout,"installed RegionMarks consumer");
...
async function main() {
  ...
  output=resolve(output); if(!(output===OUTPUT_ROOT||output.startsWith(OUTPUT_ROOT+sep)))throw new Error("output must stay under .work/dist/cp4"); if(existsSync(output))throw new Error("Refusing occupied CP4 output: "+display(output));
  const metadata=packageMetadata(); const inputs=[join(PACKAGE,"package.json"),...files(join(PACKAGE,"src")),...files(join(PACKAGE,"examples")),...NOTICES,BASE,fileURLToPath(import.meta.url)].map(required); const hashes=sourceHashes(inputs);
  ...
  const starter=join(build,"procedurals-region-marks-browser");mkdirSync(starter); const source=transformSources(starter); copy(tarball,join(starter,"vendor",packed.filename)); for(const notice of NOTICES)copy(notice,join(starter,basename(notice))); write(join(starter,"package.json"),starterPackage(packed.filename)); write(join(starter,"README.md"),`# RegionMarks browser starter\n\nRun npm install, then npm start and open the printed URL. This installs the adjacent ${packed.filename} and pinned p5 2.3.2. Edit region-marks.js for the operation composition.\n`); write(join(starter,"server.mjs"),server()); for(const p of ["index.html","region-marks.js","sketch.js","server.mjs"])if(p.endsWith(".js"))run("node",["--check",join(starter,p)]);
  ...
  const starterManifest=manifest(starter,build); const zip=join(output,`procedurals-region-marks-browser-${VERSION}.zip`); run("zip",["-q","-r",zip,basename(starter)],{cwd:build,timeout:180000});
  ...
  run("npm",["ci","--ignore-scripts","--no-audit","--no-fund"],{cwd:extracted,env,timeout:180000}); const archiveConsumer=consumerSmoke(extracted,join(extracted,"node_modules/@procedurals/javascript")); if(JSON.stringify(consumer.geometry_fingerprints)!==JSON.stringify(archiveConsumer.geometry_fingerprints))throw new Error("Extracted consumer geometry differs"); rmSync(join(extracted,"node_modules"),{recursive:true,force:true});
  stable(hashes); const report={status:"passed",scope:"CP4 JavaScript 0.4.0 local package and installed RegionMarks starter checks only; no browser launch, render, registry publication, or target acceptance.",package_version:VERSION,input_sha256:hashes,artifacts:{npm_tarball:{path:display(tarball),sha256:hash(tarball),entries:unpackedManifest},browser_starter_zip:{path:display(zip),sha256:hash(zip),entries:starterManifest}},installed_consumer:{region_marks:consumer,extracted_archive:archiveConsumer,p5_version:"2.3.2",package_lock_sha256:hash(join(starter,"package-lock.json")),starter_rewrites:source.rewrites},included_file_manifest:{staged_package:stagedManifest,starter:starterManifest},environment:{node:process.version,npm:run("npm",["--version"],{env,timeout:30000}).stdout.trim()}};
