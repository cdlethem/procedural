/** Serve the editable p5 BlurMarks example locally using the pinned, repository-local runtime. */
...
export async function startBlurMarksServer(port=8781) {
  ...
  const server=http.createServer(async(req,res)=>{
    try {
      ...
      let relative;
      if(pathname==='/'){res.writeHead(302,{Location:'/packages/javascript/examples/blur-marks/index.html'});res.end();return;}
      if(pathname==='/p5.js')relative='.work/environments/p5js/node_modules/p5/lib/p5.min.js';
      else if(/^\/packages\/javascript\/(src\/(internal\/)?[a-z0-9-]+\.js|examples\/blur-marks\/(index\.html|sketch\.js|blur-marks\.js))$/.test(pathname))relative=pathname.slice(1);
...
if(process.argv[1]&&path.resolve(process.argv[1])===fileURLToPath(import.meta.url)) {
  const server=await startBlurMarksServer();
  console.log(`Blur marks: http://127.0.0.1:${server.address().port}/ — Ctrl-C to stop`);
