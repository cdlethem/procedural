#!/usr/bin/env python3
"""Run the preregistered RegionMarks native probe on an existing API33 emulator."""
import argparse
import hashlib
import json
import subprocess
...
from build_android_region_marks import ROOT, SDK, digest, staged_hashes

APP = "org.procedurals.regionmarksprobe"
ACTIVITY = "org.procedurals.examples.regionmarks.RegionMarksProbeActivity"
...
report = {"status": "failed", "scope": "API33 RegionMarks native callbacks, pause/resume and cached MediaStore save", "build": metadata,
          "runner_sha256": digest(Path(__file__)),
          "plan_sha256": digest(ROOT / "design/capabilities/region-marks-android-acceptance.md")}
start = time.monotonic()

def adb(*arguments, cleanup=False, check=True):
    left = (240 if cleanup else 210) - (time.monotonic() - start)
    if left <= 0: raise TimeoutError("RegionMarks execution deadline")
...
try:
    ...
    def capture_screen(name):
        ...
        return target
    while True:
        result = adb("exec-out", "run-as", APP, "cat", "files/region-marks-probe/result.json", check=False)
        ...
        if result.returncode == 0:
            ...
            if state["status"] == "awaiting-pause" and not resumed:
                ...
                if len(bounds)!=4 or not all(isinstance(value,int) for value in bounds) or bounds[2]<=bounds[0] or bounds[3]<=bounds[1]:
                    raise RuntimeError("missing RegionMarks viewport bounds")
                time.sleep(1.0)
            ...
            if state["status"] == "resumed-awaiting-continue" and resumed and not continued:
                ...
                adb("shell","run-as",APP,"touch","files/region-marks-probe/continue")
                continued=True
            if state["status"] == "passed": break
        time.sleep(0.2)
    if not resumed or not continued or not state['paused'] or not state['resumed'] or state['composition_count'] != 7:
        raise RuntimeError("incomplete lifecycle/sequence")
    report['native'] = state
    report['images'] = {}
    for name in [row['id'] + '.png' for row in state['frames']] + ['saved.png']:
        data = adb('exec-out', 'run-as', APP, 'cat', 'files/region-marks-probe/' + name).stdout
    ...
    if report['images']['saved.png'] != report['images']['authored.png']:
        raise RuntimeError('saved image differs')
    verify()
    report['status'] = 'passed'
except Exception as error:
    report['failure'] = str(error)
    failed = adb('exec-out', 'run-as', APP, 'cat', 'files/region-marks-probe/result.json', cleanup=True, check=False)
