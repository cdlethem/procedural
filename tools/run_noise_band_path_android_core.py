#!/usr/bin/env python3
"""Run prepared noise-band-path vectors on a live API33 ART runtime; no drawing or APK install.
...
def verify_prepared(directory):
    ...
    if set(manifest['generated_sources_sha256']) != {'NoiseBandPathVectors.java', 'NoiseBandPathAndroidAccess.java'}:
...
def main():
    ...
    remote = '/data/local/tmp/procedurals-noise-band-path-' + digest(dex)[:16] + '-' + str(time.monotonic_ns()) + '.dex'
    report = {'status': 'failed', 'scope': 'API33 ART pure noise-band-path vectors and ownership/access only; no renderer or performance claim.',
    ...
    try:
        ...
        if report['results']['NoiseBandPathVectors'].get('cases') != 18:
            raise RuntimeError('incomplete shared fixture execution')
        if report['results']['NoiseBandPathAndroidAccess'].get('checks') != 50:
            raise RuntimeError('ownership/access checks missing')
        if report['results']['NoiseBandPathAndroidAccess'].get('native_ownership_access') is not True:
