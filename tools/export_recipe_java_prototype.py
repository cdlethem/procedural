...
def main():
 p=argparse.ArgumentParser(description=__doc__);p.add_argument('--recipe',type=Path,required=True);p.add_argument('--output',type=Path,required=True);args=p.parse_args()
 output=fresh_output(ROOT,args.output); recipe=load_json(args.recipe);validate(recipe)
