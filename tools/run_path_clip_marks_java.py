#!/usr/bin/env python3
"""Compile the current Java core and run the bounded PathClipMarks JAVA2D probe."""
...
IDS = ["baseline", "shallow-notch", "notch-restored", "recolored", "overlay", "reset"]
KEYS = "nnco0s"
...
def validate_native(native: dict, output: Path, jar: Path) -> dict:
    if native.get("status") != "passed" or native.get("frames") != len(IDS) or native.get("keys") != KEYS:
        raise RuntimeError("Incomplete PathClipMarks native sequence")
    records = native.get("frame_records")
    if not isinstance(records, list) or [item.get("id") for item in records] != IDS:
        raise RuntimeError("Unexpected PathClipMarks frame records")
    if native.get("core_code_source") != str(jar) or native.get("expected_jar") != str(jar):
        raise RuntimeError("PathClipMarks did not use the source-built core JAR")
    if native.get("renderer") != "processing.awt.PGraphicsJava2D":
        raise RuntimeError("PathClipMarks renderer was not PGraphicsJava2D")
    if any(native.get(key) != value for key, value in (("width", 640), ("height", 640), ("density", 1))):
        raise RuntimeError("PathClipMarks dimensions or density mismatch")
    ...
    names = [*IDS, "path-clip-marks"]
    images = {}
    for name in names:
        path = output / (name + ".png")
        if not path.is_file():
            raise RuntimeError("Missing PathClipMarks image: " + name)
...
def main() -> None:
    ...
    pde = ROOT / "packages/java-processing/examples/PathClipMarks/PathClipMarks.pde"
    probe = ROOT / "tests/native/PathClipMarksProbe.java"
    plan = ROOT / "design/capabilities/path-clip-marks-native-plan.md"
    ...
    commands = []
    report = {"status": "failed", "scope": "Candidate source-built core/JAR and actual PathClipMarks JAVA2D lifecycle; no distribution acceptance", "commands": commands, "input_sha256_before": before}
    ...
    try:
        ...
        candidate_jar, generated = output / "procedurals-core-candidate.jar", output / "PathClipMarks.java"
        ...
        result = invoke([java, "-Duser.home=" + str(home), "-cp", str(prep) + os.pathsep + prepath,
                         "PreprocessSketch", pde, generated, "PathClipMarks"])
        if result.returncode:
        ...
        if result.returncode:
            raise RuntimeError("PathClipMarks/probe compilation failed")
        ...
        if args.native:
            ...
            command = [sys.executable, lease, "--timeout", "120", "--", "xvfb-run", "-a", java,
                       ...
                       "PathClipMarksProbe", native_out, candidate_jar]
            result = invoke(command, timeout=150)
            if result.returncode:
                raise RuntimeError("PathClipMarks native process failed")
            if result.stderr:
                raise RuntimeError("Unexpected native stderr diagnostics: " + result.stderr)
            native_path = native_out / "native.json"
            if not native_path.is_file():
                raise RuntimeError("PathClipMarks native process produced no native.json")
