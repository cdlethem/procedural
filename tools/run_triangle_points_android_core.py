#!/usr/bin/env python3
"""Run prepared triangle vectors on a live API33 ART runtime; no drawing or APK install.
...
def verify_prepared(directory):
    ...
    if set(manifest['generated_sources_sha256']) != {'TrianglePointsVectors.java', 'TrianglePointsAndroidAccess.java'}:
...
def main():
    ...
    remote = '/data/local/tmp/procedurals-triangle-' + digest(dex)[:16] + '-' + str(time.monotonic_ns()) + '.dex'
    report = {'status': 'failed', 'scope': 'API33 ART pure triangle vectors and ownership/access only; no renderer or performance claim.',
    ...
    try:
        ...
        if report['results']['TrianglePointsVectors'].get('fixture_cases') != 59:
            raise RuntimeError('incomplete shared fixture execution')
        if report['results']['TrianglePointsAndroidAccess'].get('assertions') != 356:
            raise RuntimeError('ownership/access checks missing')
        if report['results']['TrianglePointsAndroidAccess'].get('native_ownership_access') is not True:
