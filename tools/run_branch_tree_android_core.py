#!/usr/bin/env python3
"""Run prepared branch vectors on a live API33 ART runtime; no drawing or APK install.
...
def verify_prepared(directory):
    ...
    if set(manifest['generated_sources_sha256']) != {'BranchTreeVectors.java', 'BranchTreeAndroidAccess.java'}:
...
def main():
    ...
    remote = '/data/local/tmp/procedurals-branch-' + digest(dex)[:16] + '-' + str(time.monotonic_ns()) + '.dex'
    report = {'status': 'failed', 'scope': 'API33 ART pure branch vectors and ownership/access only; no renderer or performance claim.',
    ...
    try:
        ...
        if report['results']['BranchTreeVectors'].get('fixture_cases') != 40:
            raise RuntimeError('incomplete shared fixture execution')
        if report['results']['BranchTreeAndroidAccess'].get('assertions') != 81:
            raise RuntimeError('ownership/access checks missing')
        if report['results']['BranchTreeAndroidAccess'].get('native_ownership_access') is not True:
            raise RuntimeError('ownership/access completion missing')
        if report['results']['BranchTreeAndroidAccess'].get('chain_nodes') != 513:
            raise RuntimeError('iterative chain missing')
