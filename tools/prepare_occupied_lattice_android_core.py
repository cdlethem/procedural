#!/usr/bin/env python3
"""Prepare existing occupied-lattice-paths vectors/access checks for ART; no native execution.

Compiles and preflights directly from CORE/NATIVE/vectors_source rather than
reusing run_occupied_lattice_paths_java.py's own main(), which additionally
verifies a design/phase2 + tools/check_catalog.py review-binding snapshot
unrelated to this operation's own core/fixture/native sources (see the
target-springs Android prepare script this follows and the porting
checkpoint's note on that pre-existing staleness).
"""
import argparse
import json
import os
...
from run_occupied_lattice_paths_java import ROOT, CATALOG, FIXTURE, CORE, NATIVE, digest as sha, vectors_source


def access_source():
    source = NATIVE.read_text()
    if source.count("public final class OccupiedLatticePathsNative") != 1:
        raise RuntimeError("occupied-lattice-paths access declaration changed")
    source = source.replace("public final class OccupiedLatticePathsNative", "public final class OccupiedLatticeAndroidAccess", 1)
    ...
    if source.count(marker) != 1:
        raise RuntimeError("occupied-lattice-paths success-line marker changed")
...
def main():
    ...
    paths = [CATALOG, FIXTURE, CORE, NATIVE, Path(__file__).resolve(),
             ROOT / "tools/run_occupied_lattice_paths_java.py", ROOT / "tools/prepare_circle_placement_android_core.py"]
    ...
    vector_source = output / "OccupiedLatticeVectors.java"
    access = output / "OccupiedLatticeAndroidAccess.java"
    ...
    results = {}
    for name in ("OccupiedLatticeVectors", "OccupiedLatticeAndroidAccess"):
    ...
    if results["OccupiedLatticeVectors"]["cases"] != len(fixture["cases"]):
    ...
    report = {"status": "passed", "scope": "Desktop occupied-lattice-paths fixtures/access and API33 dex preparation only; no ART/native support claim",
