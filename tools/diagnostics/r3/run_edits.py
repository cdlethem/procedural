#!/usr/bin/env python3
"""Measure one complete LandscapeMarks frame; preserve source bindings and failed attempts."""
...
probe=ROOT/'tools/diagnostics/r3/LandscapeMarksProbe.java'
build=ROOT/'.work/build/r3-edits'
out=ROOT/'.work/reproductions/r3-edits'
report=ROOT/'evidence/reproductions/r3-p2d/edits.json'
...
for p in [probe,Path(__file__),stage/'result.json',ROOT/'design/capabilities/parapara-recreation-acceptance.md']:
 bound[str(p.relative_to(ROOT))]=sha(p)
build.mkdir(parents=True)
cp=os.pathsep.join(map(str,[stage/'build',stage/'LandscapeMarks/code/procedurals.jar',*[RUNTIME/n for n in JARS]]))
...
with (ROOT/'.work/processing-render.lock').open('a') as lock:
 ...
 cmd=['xvfb-run','-a',str(jdk/'bin/java'),'-Xmx512m','-Duser.home='+str(out/'home'),'-Djava.io.tmpdir='+str(out/'tmp'),'-cp',str(build)+os.pathsep+cp,'LandscapeMarksProbe',str(out),str(stage/'LandscapeMarks/code/procedurals.jar')]
 ...
 try:
  ...
  names=['baseline','palette','reset-colour','uniform','reset-height','new-seed','final-reset']
  ...
  for n in ['reset-colour','reset-height','final-reset','landscape-marks']: assert pixels(n)==baseline,n
  for n in ['palette','uniform','new-seed']: assert pixels(n)!=baseline,n
  result['save']=png(out/'landscape-marks.png')
  for n,v in bound.items(): assert sha(ROOT/n)==v,n
