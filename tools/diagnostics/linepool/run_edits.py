#!/usr/bin/env python3
"""Measure one complete CutBranchMarks frame; preserve source bindings and failed attempts."""
...
probe=ROOT/'tools/diagnostics/linepool/CutBranchMarksProbe.java'
build=ROOT/'.work/build/line-pool-edits'
out=ROOT/'.work/reproductions/line-pool-edits'
report=ROOT/'evidence/reproductions/line-pool-p2d/edits.json'
...
for p in [probe,Path(__file__),stage/'result.json',ROOT/'design/capabilities/line-pool-workflow-acceptance.md']:
 bound[str(p.relative_to(ROOT))]=sha(p)
build.mkdir(parents=True)
cp=os.pathsep.join(map(str,[stage/'build',stage/'CutBranchMarks/code/procedurals.jar',*[RUNTIME/n for n in JARS]]))
...
with (ROOT/'.work/processing-render.lock').open('a') as lock:
 ...
 cmd=['xvfb-run','-a',str(jdk/'bin/java'),'-Xmx512m','-Duser.home='+str(out/'home'),'-Djava.io.tmpdir='+str(out/'tmp'),'-cp',str(build)+os.pathsep+cp,'CutBranchMarksProbe',str(out),str(stage/'CutBranchMarks/code/procedurals.jar')]
 plan={'owner':'root','scope':'eleven native editing/reset states and cached save','timeout_seconds':180,'input_sha256':bound,'command':cmd}
 ...
 try:
  ...
  assert native['status']=='passed' and native['frames']==11 and native['key_events']==11
  from PIL import Image
  names=['baseline','palette','reset-colour','narrow','reset-angle','sparse','reset-work','alternate','reset-stroke','new-seed','final-reset']
  ...
  for n in ['reset-colour','reset-angle','reset-work','reset-stroke','final-reset','cut-branch-marks']: assert pixels(n)==baseline,n
  for n in ['palette','narrow','sparse','alternate','new-seed']: assert pixels(n)!=baseline,n
  result['save']=png(out/'cut-branch-marks.png')
  for n,v in bound.items(): assert sha(ROOT/n)==v,n
