#!/usr/bin/env python3
"""Prepare existing closed-spline vectors/access checks for ART; no native execution."""
import argparse
import json
import os
from pathlib import Path
from prepare_circle_placement_android_core import run, require_file, java_home, d8_path, tree_hashes
from run_closed_spline_java import ROOT, CATALOG, FIXTURE, CORE, NATIVE, digest as sha, vector_source


def access_source():
    source = NATIVE.read_text()
    marker = "    private static String workload("
    if source.count(marker) != 1:
        raise RuntimeError("closed-spline access extraction marker changed")
    prefix = source[:source.index(marker)]
    for before, after in [("public final class ClosedSplineNative", "public final class ClosedSplineAndroidAccess")]:
        if prefix.count(before) != 1:
            raise RuntimeError("closed-spline access declaration changed")
...
def main():
    ...
    paths = [CATALOG, FIXTURE, CORE, NATIVE, Path(__file__).resolve(),
             ROOT / "tools/run_closed_spline_java.py", ROOT / "tools/prepare_circle_placement_android_core.py"]
    ...
    vectors = output / "ClosedSplineVectors.java"
    access = output / "ClosedSplineAndroidAccess.java"
    ...
    results = {}
    for name in ("ClosedSplineVectors", "ClosedSplineAndroidAccess"):
    ...
    if results["ClosedSplineVectors"]["cases"] != len(fixture["cases"]):
    ...
    report = {"status": "passed", "scope": "Desktop closed-spline fixtures/access and API33 dex preparation only; no ART/native support claim",
