#!/usr/bin/env python3
"""Build a CP9 candidate core and compile its editable staged sketch; no rendering."""
...
def main():
    ...
    review_path = ROOT / 'evidence/distribution/cp8-review.json'
    review = json.loads(review_path.read_text())
    core_review_path = ROOT / 'evidence/conformance/delaunay-java-root-review.json'
    core_review = json.loads(core_review_path.read_text())
    if any(r.get('status') != 'accepted' or r.get('owner') != 'root'
    ...
    if sha(archive) != review['final_archive']['sha256']:
        raise RuntimeError('Accepted CP8 archive changed')
    source = ROOT / 'packages/java/src/main/java/org/procedurals/topology/Delaunay2D.java'
    if sha(source) != core_review['implementation_sha256'][str(source.relative_to(ROOT))]:
        raise RuntimeError('Accepted Delaunay source changed')
    helper = ROOT / 'packages/java/examples/FacetMarks/FacetComposition.java'
    pde = ROOT / 'packages/java-processing/examples/FacetMarks/FacetMarks.pde'
    native = ROOT / 'tests/native/FacetCompositionNative.java'
    ...
    inputs = [Path(__file__).resolve(), review_path, core_review_path, archive,
              source, helper, pde, native, bridge, processing, *prelibs,
    ...
    try:
        ...
        if hashlib.sha256(inherited).hexdigest() != '2cd36310135e4d74f748e33cf062666d4abb6a8d9844620f921d7c0e7936440a':
            raise RuntimeError('Unexpected inherited core JAR')
        ...
        with zipfile.ZipFile(io.BytesIO(inherited)) as old, zipfile.ZipFile(library, 'x') as new:
            ...
            if not additions:
                raise RuntimeError('No Delaunay classes compiled')
            for path in additions:
                name = path.relative_to(addition).as_posix()
                if name in old_names or not name.startswith('org/procedurals/topology/Delaunay2D'):
        ...
        generated = build / 'FacetMarks.java'
        run([jdk / 'bin/java', '-Duser.home=' + str(output / 'home'), '-cp', str(build) + os.pathsep + pre,
             'PreprocessSketch', sketch / pde.name, generated, 'FacetMarks'])
        cp = os.pathsep.join(map(str, [processing, library, build]))
        run([jdk / 'bin/javac', '--release', '17', '-cp', cp, '-d', build,
             sketch / helper.name, native, generated])
        run([jdk / 'bin/java', '-Djava.awt.headless=true', '-cp', cp, 'FacetCompositionNative'])
