#!/usr/bin/env python3
"""Package the accepted R1 example, preserving the Java0.11 core and prior payloads."""
import argparse
import json
import zipfile
...
def main():
    ...
    parser.add_argument('--output', type=Path, default=ROOT/'.work/dist/r1/java')
    args = parser.parse_args()
    output = args.output.resolve()
    output.relative_to(ROOT/'.work')
    if output.exists(): raise RuntimeError('Preserve prior package attempts')
    prior_path = ROOT/'evidence/distribution/cp11-review.json'
    review_path = ROOT/'evidence/reproductions/r1-p3d/root-review.json'
    result_path = ROOT/'evidence/reproductions/r1-p3d/evaluated-result.json'
    ...
    if old[jar]!=(stage/'ReliefMarks/code/procedurals.jar').read_bytes():
        raise RuntimeError('Native JAR differs from prior package')
    additions = {
      'procedurals/examples/ReliefMarks/ReliefComposition.java':ROOT/'packages/java/examples/ReliefMarks/ReliefComposition.java',
      'procedurals/examples/ReliefMarks/ReliefMarks.pde':ROOT/'packages/java-processing/examples/ReliefMarks/ReliefMarks.pde',
      'procedurals/docs/relief-marks.md':ROOT/'docs/relief-marks.md',
      'procedurals/docs/installing-relief-marks.md':ROOT/'docs/installing-relief-marks.md'}
    for member, path in additions.items():
        safe_member(member)
        if member in old: raise RuntimeError('New example member already present')
        if '/examples/' in member and path.read_bytes()!=(stage/'ReliefMarks'/path.name).read_bytes():
    ...
    payloads=dict(old)
    payloads[version]=old[version].replace(marker,b'version=12\nprettyVersion=0.12.0\n',1)
    ...
    archive=output/'procedurals-processing-0.12.0.zip'
    ...
    if operations!=14 or starters!=12: raise RuntimeError('Unexpected delivery counts')
