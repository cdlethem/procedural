#!/usr/bin/env python3
"""Run masked source-over exact vectors and focused Java carrier checks."""
...
CORE = ROOT / "packages/java/src/main/java/org/procedurals/raster/MaskedComposite2D.java"
NATIVE = ROOT / "tests/native/MaskedCompositeNative.java"
CATALOG = ROOT / "catalog/operations/masked-source-over.json"
FIXTURE = ROOT / "fixtures/operations/masked-source-over.json"
...
def typed(case: dict) -> str:
    value = case["input"]
    source = ",".join("(int)" + str(pixel) + "L" for pixel in value["source"]["pixels"])
    destination = ",".join("(int)" + str(pixel) + "L" for pixel in value["destination"]["pixels"])
    mask = ",".join(str(number) for number in value["mask"])
    return "MaskedComposite2D.compose(%s,%s,new int[]{%s},new int[]{%s},new double[]{%s})" % (
        value["source"]["width"], value["source"]["height"], source, destination, mask)
...
def vector_source(fixture: dict) -> str:
    ...
    for index, case in enumerate(fixture["cases"]):
        call = "MaskedComposite2D.compose(" + carrier(case["input"]) + ")"
...
import java.util.*;
public final class MaskedCompositeVectors {
  ...
  static void expect(Action action,String code,String message) { try { action.run(); throw new AssertionError(message+" missing error"); } catch(MaskedComposite2D.MaskedCompositeException error) { ok(code.equals(error.code),message); } }
  %s
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / ".work/conformance/masked-source-over-java.json")
    ...
    build = Path(tempfile.mkdtemp(prefix="masked-source-over-", dir=build_root))
    vectors = build / "MaskedCompositeVectors.java"
    vectors.write_text(vector_source(fixture))
    execute([str(javac), "--release", "8", "-d", str(build), str(CORE), str(NATIVE), str(vectors)])
    vector_result = json.loads(execute([str(java), "-cp", str(build), "org.procedurals.raster.MaskedCompositeVectors"]).stdout)
    native_result = json.loads(execute([str(java), "-cp", str(build), "org.procedurals.raster.MaskedCompositeNative"]).stdout)
    after = {str(path.relative_to(ROOT)): digest(path) for path in bound}
    if before != after:
        raise RuntimeError("bound input changed during check")
    report = {
        "status": "passed", "operation": "raster.masked-source-over-2d",
