#!/usr/bin/env python3
"""Run prepared seeded-line-pool vectors on a live API33 ART runtime; no drawing or APK install.
...
def verify_prepared(directory):
    ...
    if set(manifest['generated_sources_sha256']) != {'LinePoolVectors.java', 'LinePoolAndroidAccess.java'}:
...
def main():
    ...
    remote = '/data/local/tmp/procedurals-line-pool-' + digest(dex)[:16] + '-' + str(time.monotonic_ns()) + '.dex'
    report = {'status': 'failed', 'scope': 'API33 ART pure seeded-line-pool vectors and ownership/access only; no renderer or performance claim.',
    ...
    try:
        ...
        expected_cases = prepared['desktop_preflight']['LinePoolVectors']['cases']
        expected_assertions = prepared['desktop_preflight']['LinePoolAndroidAccess']['assertions']
        if report['results']['LinePoolVectors'].get('cases') != expected_cases:
            raise RuntimeError('incomplete shared fixture execution')
        if report['results']['LinePoolAndroidAccess'].get('assertions') != expected_assertions:
            raise RuntimeError('ownership/access checks missing')
        if report['results']['LinePoolAndroidAccess'].get('native_ownership_access') is not True:
