#!/usr/bin/env python3
"""Package the accepted R2 example, preserving the Java0.12 core and prior payloads."""
import argparse
import json
import zipfile
...
def main():
    ...
    parser.add_argument('--output', type=Path, default=ROOT/'.work/dist/r2/java')
    args = parser.parse_args()
    output = args.output.resolve()
    output.relative_to(ROOT/'.work')
    if output.exists(): raise RuntimeError('Preserve prior package attempts')
    prior_path = ROOT/'evidence/distribution/r1-review.json'
    review_path = ROOT/'evidence/reproductions/r2-p3d/root-review.json'
    result_path = ROOT/'evidence/reproductions/r2-p3d/edits.json'
    ...
    if old[jar]!=(stage/'CityMarks/code/procedurals.jar').read_bytes():
        raise RuntimeError('Native JAR differs from prior package')
    additions = {
      'procedurals/examples/CityMarks/CityComposition.java':ROOT/'packages/java/examples/CityMarks/CityComposition.java',
      'procedurals/examples/CityMarks/CityMarks.pde':ROOT/'packages/java-processing/examples/CityMarks/CityMarks.pde',
      'procedurals/docs/city-marks.md':ROOT/'docs/city-marks.md',
      'procedurals/docs/installing-city-marks.md':ROOT/'docs/installing-city-marks.md'}
    for member, path in additions.items():
        safe_member(member)
        if member in old: raise RuntimeError('New example member already present')
        if '/examples/' in member and path.read_bytes()!=(stage/'CityMarks'/path.name).read_bytes():
    ...
    payloads=dict(old)
    payloads[version]=old[version].replace(marker,b'version=13\nprettyVersion=0.13.0\n',1)
    ...
    archive=output/'procedurals-processing-0.13.0.zip'
    ...
    if operations!=14 or starters!=13: raise RuntimeError('Unexpected delivery counts')
