#!/usr/bin/env python3
"""Build a R2 unchanged core and compile its editable staged sketch; no rendering."""
...
def main():
    ...
    review_path = ROOT / 'evidence/distribution/r1-review.json'
    ...
    if sha(archive) != review['final_archive']['sha256']:
        raise RuntimeError('Accepted Java0.12 archive changed')
    helper = ROOT / 'packages/java/examples/CityMarks/CityComposition.java'
    pde = ROOT / 'packages/java-processing/examples/CityMarks/CityMarks.pde'
    ...
    try:
        ...
        generated = build / 'CityMarks.java'
        run([jdk / 'bin/java', '-Duser.home=' + str(output / 'home'), '-cp', str(build) + os.pathsep + pre,
             'PreprocessSketch', sketch / pde.name, generated, 'CityMarks'])
