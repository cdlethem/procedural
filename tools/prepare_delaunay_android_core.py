#!/usr/bin/env python3
"""Prepare existing delaunay vectors/access checks for ART; no native execution."""
import argparse
import json
import os
from pathlib import Path
from prepare_circle_placement_android_core import run, require_file, java_home, d8_path, tree_hashes
from check_delaunay_java import ROOT, CATALOG, FIXTURE, CORE, NATIVE, sha, vectors_source


def access_source():
    source = NATIVE.read_text()
    for before, after in [("public final class Delaunay2DNative", "public final class Delaunay2DAndroidAccess"),
                          ('"{\\"status\\":\\"passed\\",\\"assertions\\":"',
                           '"{\\"status\\":\\"passed\\",\\"native_ownership_access\\":true,\\"assertions\\":"')]:
        if source.count(before) != 1:
            raise RuntimeError("delaunay access declaration changed")
...
def main():
    ...
    paths = [CATALOG, FIXTURE, CORE, NATIVE, Path(__file__).resolve(),
             ROOT / "tools/check_delaunay_java.py", ROOT / "tools/prepare_circle_placement_android_core.py"]
    ...
    vectors = output / "Delaunay2DVectors.java"
    access = output / "Delaunay2DAndroidAccess.java"
    ...
    results = {}
    for name in ("Delaunay2DVectors", "Delaunay2DAndroidAccess"):
    ...
    if results["Delaunay2DVectors"]["fixture_cases"] != len(fixture["cases"]):
    ...
    report = {"status": "passed", "scope": "Desktop delaunay fixtures/access and API33 dex preparation only; no ART/native support claim",
