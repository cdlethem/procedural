...
import { rasterCrossfade2D } from "../../packages/javascript/src/raster-crossfade.js";

const ROOT = resolve(dirname(fileURLToPath(import.meta.url)), "../..");
const CATALOG = join(ROOT, "catalog/operations/raster-crossfade.json");
const FIXTURE = join(ROOT, "fixtures/operations/raster-crossfade.json");
const SOURCE = join(ROOT, "packages/javascript/src/raster-crossfade.js");
...
function checkCase(item) {
  if (!item.error) {
    const result = rasterCrossfade2D(item.input);
    assert.deepEqual(result.toValues(), item.output, item.id);
    return result;
  }
  assert.throws(() => rasterCrossfade2D(item.input), (error) => {
...
function ownershipChecks() {
  const config = { first: { width: 1, height: 1, pixels: [0xff112233] }, second: { width: 1, height: 1, pixels: [0xffaabbcc] }, weights: [0.5] };
  const result = rasterCrossfade2D(config), baseline = result.toValues();
  config.first.pixels[0] = 0; config.weights[0] = 0; config.weights.push(1);
...
function nativeChecks() {
  const base = { first: { width: 1, height: 1, pixels: [0xff112233] }, second: { width: 1, height: 1, pixels: [0xffaabbcc] }, weights: [0.5] };
  errorCode(() => rasterCrossfade2D(null), "INVALID_INPUT");
  errorCode(() => rasterCrossfade2D({ ...base, extra: 1 }), "INVALID_INPUT");
  errorCode(() => rasterCrossfade2D({ ...base, second: { width: 2, height: 1, pixels: [1, 2] } }), "INVALID_INPUT");
  errorCode(() => rasterCrossfade2D({ ...base, weights: [0.5, 0.5] }), "INVALID_INPUT");
  errorCode(() => rasterCrossfade2D({ ...base, weights: [-0.1] }), "INVALID_INPUT");
  errorCode(() => rasterCrossfade2D({ ...base, weights: [1.1] }), "INVALID_INPUT");
  errorCode(() => rasterCrossfade2D({ ...base, first: { width: 1, height: 1, pixels: [4294967296] } }), "INVALID_INPUT");
  errorCode(() => rasterCrossfade2D({ ...base, first: { width: 0, height: 1, pixels: [] } }), "INVALID_INPUT");
  // Negative-zero weight equals zero: first-hidden identity branch.
  const nz = rasterCrossfade2D({ ...base, weights: [-0] });
  assert.deepEqual(nz.toValues().pixels, [0xff112233]);
