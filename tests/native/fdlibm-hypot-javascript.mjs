...
const files=["packages/javascript/src/internal/fdlibm-hypot.js","tests/native/fdlibm-hypot-javascript.mjs","tests/native/HypotOracle.java"].map(p=>join(ROOT,p));
...
const BOUNDARIES = [
  ["5f300000ffffffff", "5f200000ffffffff", "5f31e378b9cf6e60"],
  ["5f30000100000000", "5f20000100000000", "5f31e378b9cf6e61"],
  ["20afffffffffffff", "209fffffffffffff", "20b1e3779b97f4a7"],
  ["20b0000000000000", "20a0000000000000", "20b1e3779b97f4a8"],
  ["20b0000000000001", "20a0000000000001", "20b1e3779b97f4a9"],
  ["0000000000000001", "0000000000000000", "0000000000000001"],
  ["000fffffffffffff", "0008000000000000", "0011e3779b97f4a7"],
  ["0010000000000000", "0008000000000000", "0011e3779b97f4a8"],
  ["7fefffffffffffff", "7fdfffffffffffff", "7ff0000000000000"],
];
...
run(join(jdk,"bin/javac"),["--release","8","-d",classes,join(ROOT,"tests/native/HypotOracle.java")]);
const expected=run(join(jdk,"bin/java"),["-cp",classes,"HypotOracle"],pairs.map(p=>p.join(" ")).join("\n")+"\n").trim().split(/\r?\n/);
assert.equal(expected.length,pairs.length);
for(let i=0;i<pairs.length;i++){const [x,y]=pairs[i];assert.equal(bits(fdlibmHypot(value(x),value(y))),expected[i],`pair ${i}: ${x},${y}`);if(i<BOUNDARIES.length)assert.equal(expected[i],BOUNDARIES[i][2],"boundary oracle");}
const after=hashes();assert.deepEqual(after,before);
writeFileSync(output,JSON.stringify({status:"passed",scope:"Fresh Java StrictMath runtime oracle, exact binary64 comparisons for finite inputs; no implementation-source copy.",input_sha256_before:before,input_sha256_after:after,scenarios:{finite_scaling_boundaries:BOUNDARIES.length,deterministic_full_range:512,same_exponent:512,total:pairs.length},java_release:readFileSync(join(jdk,"release"),"utf8"),oracle_input_sha256:createHash("sha256").update(JSON.stringify(pairs)).digest("hex")},null,2)+"\n",{flag:"wx"});
