...
class SourceBundleTests(unittest.TestCase):
    ...
    def test_current_release_admission_and_exact_tabs(self):
        ...
        self.assertEqual(manifest['processing_version'], 35)
        self.assertEqual(len(manifest['operation_files']), 31)
        self.assertEqual(len(manifest['core_sources']), 32)
        self.assertEqual(len(manifest['adapter_sources']), 6)
        self.assertEqual(len(manifest['core_sources']) + len(manifest['adapter_sources']), 38)
        self.assertIn('processing_core_sha256', manifest)
        self.assertEqual(len(manifest['examples']), 55)
        self.assertEqual(sum(name.endswith('.pde') for name in manifest['examples']), 37)
