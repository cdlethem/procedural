...
class SourceBundleTests(unittest.TestCase):
    ...
    def test_current_release_admission_and_exact_tabs(self):
        ...
        self.assertEqual(manifest['processing_version'], 28)
        self.assertEqual(len(manifest['operation_files']), 26)
        self.assertEqual(len(manifest['core_sources']), 26)
        self.assertEqual(len(manifest['adapter_sources']), 4)
        self.assertIn('processing_core_sha256', manifest)
        self.assertEqual(len(manifest['examples']), 47)
        self.assertEqual(sum(name.endswith('.pde') for name in manifest['examples']), 29)
