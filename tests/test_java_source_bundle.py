...
class SourceBundleTests(unittest.TestCase):
    ...
    def test_current_release_admission_and_exact_tabs(self):
        ...
        self.assertEqual(manifest['processing_version'], 29)
        self.assertEqual(len(manifest['operation_files']), 27)
        self.assertEqual(len(manifest['core_sources']), 27)
        self.assertEqual(len(manifest['adapter_sources']), 6)
        self.assertIn('processing_core_sha256', manifest)
        self.assertEqual(len(manifest['examples']), 49)
        self.assertEqual(sum(name.endswith('.pde') for name in manifest['examples']), 31)
