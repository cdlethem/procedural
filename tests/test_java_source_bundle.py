...
class SourceBundleTests(unittest.TestCase):
    ...
    def test_current_release_admission_and_exact_tabs(self):
        ...
        self.assertEqual(manifest['processing_version'], 30)
        self.assertEqual(len(manifest['operation_files']), 28)
        self.assertEqual(len(manifest['core_sources']), 28)
        self.assertEqual(len(manifest['adapter_sources']), 6)
        self.assertIn('processing_core_sha256', manifest)
        self.assertEqual(len(manifest['examples']), 50)
        self.assertEqual(sum(name.endswith('.pde') for name in manifest['examples']), 32)
